...
public class genTestMatrix {
  ...
  public static void randomizeCol(float[] columnData) {
    for (int i = 0; i < columnData.length; i++) {
      if (rnd.nextFloat() < .85) columnData[i] = (rnd.nextFloat() * 2) - 1;
    }
  }

  public static void randomizeColSmall(float[] columnData) {
    for (int i = 0; i < columnData.length; i++) {
      if (rnd.nextFloat() < .10) columnData[i] = (rnd.nextFloat() * 2) - 1;
    }
  }
  ...
  public static void main(String[] args) throws Exception {
    ...
    BufferedWriter write = new BufferedWriter(
      new FileWriter("C:\\NGCHMProto\\" + filename + "\\" + filename + ".txt")
    );
    ...
    for (int i = 0; i < NUM_COLS; i++) {
      for (int j = 0; j < NUM_ROWS; j++) {
        if (rnd.nextFloat() < .3) matrix[j][i] = (rnd.nextFloat() * 2) - 1;
        else matrix[j][i] = columnData[j] + (rnd.nextFloat() * 0.6F - 0.3F);
      }
    ...
    write = new BufferedWriter(
      new FileWriter("C:\\NGCHMProto\\" + filename + "\\Smoker_ColClassification.txt")
    );
    write.write("Discrete" + "\n");
    for (int i = 0; i < NUM_COLS; i++) {
      String cat;
      if (i < (NUM_COLS / .4)) cat = (rnd.nextFloat() < .85) ? "Smoker" : "Non-Smoker";
      else cat = (rnd.nextFloat() < .15) ? "Smoker" : "Non-Smoker";
      write.write("TCGA_SAMP_" + (i + 1) + "\t" + cat + "\n");
    }
    write.close();

    write = new BufferedWriter(
      new FileWriter("C:\\NGCHMProto\\" + filename + "\\Age_ColClassification.txt")
    );
    ...
    write = new BufferedWriter(
      new FileWriter("C:\\NGCHMProto\\" + filename + "\\Type_RowClassification.txt")
    );
    write.write("Discrete" + "\n");
    for (int i = 0; i < NUM_ROWS; i++) {

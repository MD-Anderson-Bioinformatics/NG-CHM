...
public class InputClass {
  ...
  private Map<String, String> extractClassDataFromMatrix(InputFile iFile) throws Exception {
    ...
    try {
      ...
      System.out.println(
        "ERROR extracting class data from matrix for the Class: (" +
        name +
        ")  Message: " +
        ex.toString()
      );
  ...
  private String[] reOrderClassificationFile(Map<String, String> origData, String[] order)
    throws Exception {
    ...
    try {
      ...
      if (!foundMatch) {
        throw new Exception(
          "COVARIATE INVALID: The covariate file (" +
          name +
          ") contains class labels that do not match the data matrix labels"
        );
      }
    } catch (Exception ex) {
      throw ex;
  ...
  private void calcLowHighBounds(boolean lowFound, boolean highFound) throws Exception {
    try {
      ...
      System.out.println(
        "ERROR Calculating low and high boundaries for continuous class bar (" +
        name +
        "): " +
        ex.toString()
      );
  ...
  public void createClassSummaryImg(int size) throws Exception {
    ...
    try {
      if (!barType.equals(COLOR_PLOT)) {
        ...
        image = position.equals("row")
          ? new BufferedImage((int) barHeight, size, BufferedImage.TYPE_INT_RGB)
          : new BufferedImage(size, (int) barHeight, BufferedImage.TYPE_INT_RGB);
        ...
        image = position.equals("row")
          ? new BufferedImage(1, size, BufferedImage.TYPE_INT_RGB)
          : new BufferedImage(size, 1, BufferedImage.TYPE_INT_RGB);
        for (int i = 1; i < size + 1; i++) {
          int valPos = i * sampleFactor > orderedClass.length - 1
            ? orderedClass.length - 1
            : i * sampleFactor;
  ...
  public void generateClassMatrix(int length, float barHeight) throws Exception {
    ...
    for (int k = 1; k <= length; k++) {
      ...
      int valHeight = Math.round(barHeight * valScale) == barHeight
        ? Math.round(barHeight * valScale) - 1
        : Math.round(barHeight * valScale);

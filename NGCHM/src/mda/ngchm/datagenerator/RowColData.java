...
public class RowColData {
   ...
   * configuration data.
   ******************************************************************/
  public RowColData(String type, int length, JSONObject configData, InputFile iFile)
    throws Exception {
    try {
      ...
      topItems = (JSONArray) configData.get(TOP_ITEMS) != null
        ? jsonArrayToStringArray((JSONArray) configData.get(TOP_ITEMS))
        : null;
      ...
      labelMaxLength = (String) configData.get(LABEL_MAXIMUM_LENGTH) != null
        ? (String) configData.get(LABEL_MAXIMUM_LENGTH)
        : "20";
      labelAbbrevMethod = (String) configData.get(LABEL_ABBREV_METHOD) != null
        ? (String) configData.get(LABEL_ABBREV_METHOD)
        : "END";
      setClassificationOrder(orderArr);
      String extraFile = (configData.get(EXTRA_FILE) != null)
        ? (String) configData.get(EXTRA_FILE)
        : null;
        ...
        if (treeCuts > 0) {
          if (dendroValues.size() - 1 < treeCuts) {
            String warning =
              "Less " +
              orderType +
              " dendrogram values found than Cluster-Based Gaps requested.  Lowering the number of gaps requested FROM: " +
              treeCuts +
              " TO: " +
              (dendroValues.size() - 1);
    ...
    for (int i = 0; i < propCuts.length; i++) {
      ...
      if ((val < length + 1) && (val > 0)) {
        ...
        String warning =
          ...
          " Gap value " +
          val +
          " is outside the range of heat map " +
          orderType +
          "s (1-" +
          length +
          "). Gap value disregarded.";
        System.out.println(warning);
        configWarnings.add(warning);
      }
    }
   ...
   ******************************************************************/
  private int findLocationFromIndex(int index, List<Integer> barsLeft, List<Integer> barsRight)
    throws Exception {
    if (index < 0) {
    ...
    if ((orderFile != null) && (!orderFile.equals(EMPTY))) {
      ...
      if (orderMethod.equals(ORDER_ORIGINAL)) {
        ...
        if (!Arrays.equals(orderArr, matrixOrder)) {
          throw new Exception(
            "Exception: Original Order file provided is not in the order of the matrix provided."
          );
        }
      }
    } else {
  ...
  private String[] constructOrderArrayFromMatrixData(InputFile iFile) throws Exception {
    ...
    try {
      ...
      throw new Exception(
        "Exception in RowColData.constructOrderArray: Reading Matrix " +
        orderType +
        " Headers." +
        ex.toString()
      );
  ...
  private void setClassificationOrder(String[] orderArr) throws Exception {
    ...
    for (int i = 0; i < orderArr.length; i++) {
      if (orderArr[i] == null) {
        System.out.println(
          "null entry in row or column order array (likely incorrect order file specified): index " +
          i
        );
        throw new Exception();
      }
  ...
  private String buildTopItemsArray() throws Exception {
    ...
    if ((topItems.length > 0) && (itemsFound == 0)) {
      String warning =
        "No top items found for axis: " + orderType + ".  All parameter entries skipped..";
      System.out.println(warning);
      configWarnings.add(warning);
    } else if ((topItems.length > 0) && (topItems.length != itemsFound)) {
      String warning =
        "Not all top items found for axis: " + orderType + ". Those parameter entries skipped.";
      System.out.println(warning);
      configWarnings.add(warning);
    }
  ...
  private boolean adjustTopItemLines() throws Exception {
    ...
    int adjSize = 6;
    if (classArray.length > 1000) {
      adjSize = 6 * (classArray.length / 500);
    } else if (classArray.length < 200) {
      adjSize = 2;
    }
   ...
   * of the heatmap
   ******************************************************************/
  private void createTopItemsRowImg(int size, Float sampleFactor, boolean lowSampInd)
    throws Exception {
    BufferedImage image = new BufferedImage(
      TOP_ITEMS_IMAGE_WIDTH,
      size,
    ...
    for (int i = 0; i < topItemsLines.size(); i++) {
      ...
      int startpos =
        (Math.round((new Float((int) line[0])) * sampleFactor)) - (sampleFactor.intValue() / 2);
      int endpos =
        (Math.round((new Float((int) line[1])) * sampleFactor)) - (sampleFactor.intValue() / 2);
      if (!lowSampInd) {
   ...
   * of the heatmap
   ******************************************************************/
  public void createTopItemsColImg(int size, Float sampleFactor, boolean lowSampInd)
    throws Exception {
    BufferedImage image = new BufferedImage(
      size,
      TOP_ITEMS_IMAGE_WIDTH,
  ...
  private String getLabelText(String label) throws Exception {
    ...
    if (label.length() > Integer.parseInt(labelMaxLength)) {
      if (labelAbbrevMethod.equals("END")) {
        ...
        newLabel =
          label.substring(0, (maxLength / 2 - 1)) +
          "..." +
          label.substring(label.length() - (maxLength / 2 - 2), label.length());
      } else {
        newLabel = "..." + label.substring(0, maxLength - 3);
      }
    }

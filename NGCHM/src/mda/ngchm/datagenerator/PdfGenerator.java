...
public class PdfGenerator {
  ...
  public void createHeatmapPDF(ImportData iData, boolean fullPDF) {
    ...
    try {
      ...
      if (
        (iData.rowData.getVisibleClasses().size() > 0) ||
        (iData.colData.getVisibleClasses().size() > 0)
      ) {
  ...
  public PDPageContentStream getPdfPageCustomSize(
    PDDocument doc,
    ImportData iData,
    int mapWidth,
    int mapHeight
  ...
  public void createPDFHeatmapPage(PDDocument doc, BufferedImage image, ImportData iData) {
    try {
      ...
      contentStream.drawImage(
        pdImageXObject,
        rowColPos[PDF_COL_POS],
        rowColPos[PDF_ROW_POS],
        PDF_MAP_SIZE,
        PDF_MAP_SIZE
      );
  ...
  public void createFullPDFHeatmapPage(PDDocument doc, BufferedImage image, ImportData iData) {
    try {
      ...
      int[] rowColPos = getStartingPositions(
        iData,
        PDF_CONTENT_START + PDF_DENDRO_HEIGHT + mapHeight + 300
      );
      //Draw row dendrogram on PDF
      if (iData.colData.dendroMatrix != null) {
  ...
  public void createPDFDataDistributionPlot(PDDocument doc, ImportData iData) {
    try {
      ...
      histStartY -= 15;
      PDImageXObject pdMatrixDistributionLegend = LosslessFactory.createFromImage(
        doc,
        iData.matrixFiles.get(0).distributionLegend
      );
      ...
      for (int i = 0; i < threshNums; i++) {
        ...
        writePDFText(
          contentStream,
          "n = " + Integer.toString(countBins[i]),
          12,
          PDF_FONT,
          55 + ((110 * countBins[i]) / maxCount) + 2,
          histStartY - (i + 1) * 10 + 1,
          false
        );
      ...
      writePDFText(
        contentStream,
        "n = " + Integer.toString(countBins[binNums - 1]),
        12,
        PDF_FONT,
        55 + ((110 * countBins[binNums - 1]) / maxCount) + 2,
        histStartY - (threshNums + 1) * 10 + 1,
        false
      );
      writePDFText(contentStream, "Missing", 12, PDF_FONT, 5, histStartY - 120 + 10, false);
      writePDFText(
        contentStream,
        "n = " + Integer.toString(missingCount),
        12,
        PDF_FONT,
        55 + ((110 * missingCount) / maxCount) + 2,
        histStartY - (binNums + 1) * 10 + 1,
        false
      );
      ...
      if (rcFiles.size() > 0) {
        ...
        for (int i = 0; i < pairs; i++) {
          ...
          if (legendFitsOnPage(doc, rcFiles, contentStream, next, legend2, rowCovStartY)) {
            ...
            writePDFText(
              contentStream,
              "Row Covariate Bar Legends (continued)",
              12,
              PDF_FONT_BOLD,
              10,
              rowCovStartY,
      ...
      if (ccFiles.size() > 0) {
        ...
        for (int i = 0; i < pairs; i++) {
          ...
          if (legendFitsOnPage(doc, ccFiles, contentStream, next, legend2, rowCovStartY)) {
            ...
            writePDFText(
              contentStream,
              "Column Covariate Bar Legends (continued)",
              12,
              PDF_FONT_BOLD,
              10,
              rowCovStartY,
  ...
  public int writeLegend(
    PDDocument doc,
    List<InputClass> classFiles,
    PDPageContentStream contentStream,
    int legend1,
    int legend2,
    int rowCovStartY,
    PDImageXObject pdImageBorderBox
  ) {
    int legend1Rows = 0;
    try {
  ...
  public boolean legendFitsOnPage(
    PDDocument doc,
    List<InputClass> classFiles,
    PDPageContentStream contentStream,
    int legend1,
  ...
  public void writeDiscreteClassTotals(
    PDPageContentStream contentStream,
    InputClass iFile,
    String[] covValues,
    int rowCovStartY,
  ...
  public void writeContinuousCovariateClassTotals(
    PDPageContentStream contentStream,
    InputClass iFile,
    String[] covValues,
    int rowCovStartY,
    ...
    try {
        ...
        if ((elemValue == null) || (NA_VALUES.contains(elemValue))) {
          ...
          for (int k = 0; k < iFile.map.contBreaks.size(); k++) {
            if (k == 0 && Float.valueOf(elemValue) < Float.valueOf(iFile.map.contBreaks.get(k))) {
              ...
              k == iFile.map.contBreaks.size() - 1 &&
              Float.valueOf(elemValue) > Float.valueOf(iFile.map.contBreaks.get(k))
            ) {
              covTotals[k]++;
      ...
      System.out.println(
        "Exception in PdfGenerator.writeContinuousCovariateClassTotals: " + ex.toString()
      );
      ex.printStackTrace();
    }
  }

  public void drawHeaderFooter(
    PDDocument doc,
    PDPageContentStream contentStream,
    String mapName,
    int pageWidth,
  ...
  public int[] drawColumnDendrogram(
    PDDocument doc,
    PDPageContentStream contentStream,
    ImportData iData,
    int[] posArray,
    ...
    try {
      PDImageXObject pdColDendroImageXObject = LosslessFactory.createFromImage(
        doc,
        iData.colData.dendroImage
      );
      contentStream.drawImage(
        pdColDendroImageXObject,
        posArray[PDF_COL_POS],
        posArray[PDF_ROW_POS],
        mapWidth,
        PDF_DENDRO_HEIGHT
      );
    } catch (Exception ex) {
  ...
  public int[] drawColumnCovariates(
    PDDocument doc,
    PDPageContentStream contentStream,
    ImportData iData,
    int[] posArray,
    ...
    try {
      ...
      for (int i = 0; i < icList.size(); i++) {
        ...
        if (!ic.barType.equals(COLOR_PLOT)) {
          ...
          horizPos += 3;
          String midStr = String.valueOf(
            Math.round(
              Float.parseFloat(ic.lowBound) +
              ((Float.parseFloat(ic.highBound) - Float.parseFloat(ic.lowBound)) / 2)
            )
          );
          String lowStr = Float.parseFloat(ic.highBound) >= 2
            ? ic.lowBound.replaceFirst("\\.0*$|(\\.\\d*?)0+$", "$1")
            : ic.lowBound;
          String highStr = Float.parseFloat(ic.highBound) >= 2
            ? ic.highBound.replaceFirst("\\.0*$|(\\.\\d*?)0+$", "$1")
            : ic.highBound;
          midStr = Float.parseFloat(ic.highBound) >= 2
            ? midStr.replaceFirst("\\.0*$|(\\.\\d*?)0+$", "$1")
            : midStr;
  ...
  public int[] drawRowDendrogram(
    ...
    try {
      PDImageXObject pdRowDendroImageXObject = LosslessFactory.createFromImage(
        doc,
        iData.rowData.dendroImage
      );
      contentStream.drawImage(
        pdRowDendroImageXObject,
        10,
        posArray[PDF_ROW_POS],
        PDF_DENDRO_HEIGHT,
        mapHeight
  ...
  public int[] drawRowTopItems(
    ...
    try {
      ...
      for (int i = 0; i < iData.rowData.topItemsLines.size(); i++) {
        ...
        int textLoc =
          Math.round(((int) iData.rowData.topItemsLines.get(i)[1]) * increment) +
          2 -
  ...
  public int[] drawColTopItems(
    ...
    try {
      ...
      for (int i = 0; i < iData.colData.topItemsLines.size(); i++) {
        ...
        int textLoc =
          Math.round(((int) iData.colData.topItemsLines.get(i)[1]) * increment) -
          2 -
          (increment.intValue() / 2);
        if (itemVal != null) {
          writePDFText(
    ...
    try {
      ...
      for (int i = 0; i < text.length(); i++) {
        ...
        if (Character.UnicodeBlock.of(c) != Character.UnicodeBlock.BASIC_LATIN) {
          System.out.println(
            " PDFBox: Unprintable character found in text: " + text + ". Converting to ??."
          );
          sb.append("??");
        } else {
          sb.append(c);

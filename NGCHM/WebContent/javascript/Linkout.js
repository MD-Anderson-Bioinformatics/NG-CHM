...
(function() {
        ...
        NgChm.LNK.getLabelsByType = function(axis, linkout) {
            ...
            var labels = axis == 'Row' ? NgChm.heatMap.getRowLabels()["labels"] : axis == "Column" ? NgChm.heatMap.getColLabels()['labels'] :
                axis == "ColumnCovar" ? NgChm.heatMap.getColClassificationConfigOrder() : axis == "RowCovar" ? NgChm.heatMap.getRowClassificationConfigOrder() : [NgChm.heatMap.getRowLabels()["labels"], NgChm.heatMap.getColLabels()['labels']];
        ...
        */
        function getDiscCovariateColors(axis, label, values, colorMapMgr) {
            ...
            values.forEach(val => {
                ...
                if (tmpColor.length != 1) { console.error('Error getting color for discrete covariate');
        ...
        */
        ...
        function newGearDialog(icon, paneId) {
            ...
            /** Add covariate options to dropdown
            	...
            	selectedElementsOption Text
            	for dropdown to select rows / columns w / GRAB / SHOW buttons.If null, then
            	no option will be given
            	for using GRAB / SHOW
            	for selection in gear menu.
            	@param {
            		Boolean
            	}
            	onlyContinuous
            	if true, only add continuous covariates to dropdown.
            	@return {
            		optionNode
            	}
            	selOpt Option
            	for selecting rows / columns w / GRAB / SHOW buttons(see
            		function optionNode()).
            */
                            ...
                            if (isGrabberSelected()) {
                                ...
                                if (typeContOrDisc === 'continuous') {
                                    ...
                                    if (lastApplied[cid].hasOwnProperty('rangeStrings') && lastApplied[cid].covariate == selectedValue) {
                                        ...
                                        sss[cid].rangeSelectors.forEach((s) => { let e = new Event('change');
                                            s.element.lastChild.dispatchEvent(e) }); // <-- trigger onchange event
                                    }
                                    ...
                                    if (lastApplied[cid].hasOwnProperty('discreteCheckboxes') && lastApplied[cid].covariate == selectedValue) {
                                        ...
                                        sss[cid].discreteSelectors.forEach((s) => { let e = new Event('change');
                                            s.element.lastChild.lastChild.dispatchEvent(e) }); // trigger onchange
                                    }
    ...
    NgChm.DET.updateDisplayedLabels(); NgChm.SUM.redrawSelectionMarks(); NgChm.SEL.updateSelection(); NgChm.SRCH.showSearchResults(); NgChm.LNK.postSelectionToLinkouts(axis, msg.selection.clickType, 0, nonce);
}

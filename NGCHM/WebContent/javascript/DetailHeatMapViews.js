(function () {
  ...
  function getMatchingMap(matches) {
    ...
    if (DVW.primaryMap !== null) {
      console.warn(
        "Cannot find the required detail map. Returning the primary detail map.",
      );
      return DVW.primaryMap;
    }
    console.error(
      "Cannot find the required detail map and there is no primary detail map. Returning null.",
    );
    return null;
  }
  ...
  DVW.getMapItemFromDendro = function (dendro) {
    return getMatchingMap(
      (mapItem) => mapItem.rowDendro === dendro || mapItem.colDendro == dendro,
    );
  };
  ...
  DVW.getCurrentSumRow = function (mapItem) {
    ...
    const rowSummaryRatio = mapItem.heatMap.getRowSummaryRatio(
  ...
  DVW.getCurrentSumCol = function (mapItem) {
    ...
    const colSummaryRatio = mapItem.heatMap.getColSummaryRatio(
      MAPREP.SUMMARY_LEVEL,
    );
  ...
  DVW.getCurrentDetRow = function (mapItem) {
    ...
    if (mapItem.mode == "RIBBONV" && mapItem.selectedStart >= 1) {
      const rvRatio = mapItem.heatMap.getRowSummaryRatio(
  ...
  DVW.getCurrentDetCol = function (mapItem) {
    ...
    if (mapItem.mode == "RIBBONH" && mapItem.selectedStart >= 1) {
      const rhRatio = mapItem.heatMap.getColSummaryRatio(
  ...
  DVW.getCurrentSumDataPerRow = function (mapItem) {
    const rowSummaryRatio = mapItem.heatMap.getColSummaryRatio(
      MAPREP.SUMMARY_LEVEL,
    );
  ...
  DVW.getCurrentSumDataPerCol = function (mapItem) {
    const colSummaryRatio = mapItem.heatMap.getRowSummaryRatio(
      MAPREP.SUMMARY_LEVEL,
    );
  ...
  DVW.setCurrentRowFromSum = function (mapItem, sumRow) {
    ...
    mapItem.currentRow =
      sumRow * mapItem.heatMap.getRowSummaryRatio(MAPREP.SUMMARY_LEVEL);
  ...
  DVW.setCurrentColFromSum = function (mapItem, sumCol) {
    ...
    mapItem.currentCol =
      sumCol * mapItem.heatMap.getColSummaryRatio(MAPREP.SUMMARY_LEVEL);
  ...
  DVW.checkRow = function (mapItem) {
    ...
    if (
      ...
      (mapItem.mode == "RIBBONV" && mapItem.selectedStart == 0)
    ) {
      mapItem.currentRow = 1;
    }
    if (
  ...
  DVW.checkCol = function (mapItem) {
    ...
    if (
      ...
      (mapItem.mode == "RIBBONH" && mapItem.selectedStart == 0)
    ) {
      mapItem.currentCol = 1;
    }
    if (

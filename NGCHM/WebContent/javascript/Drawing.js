(function () {
    ...
    function resize(width, height) {
      ...
      if (width !== this.width || height !== this.height) {
        ...
        this.pixels = new Uint8Array(
          new ArrayBuffer(width * DRAW.BYTE_PER_RGBA * height),
        );
      }
    }
    ...
    check(userinit) {
      ...
      if (this._state.lost) {
        ...
        ctx.viewport(
          0,
          0,
          ctx.drawingBufferWidth * this._widthScale,
          ctx.drawingBufferHeight * this._heightScale,
        );
        this._OK = true;
        initialized = true;
      }
      ...
      if (this._OK && !initialized) {
        ...
        if (ctx) {
          ...
          if (this._program) {
            ...
            if (
              userinit &&
              !userinit(this, this._state.context, this._program)
            ) {
  ...
  DRAW.GL.createGlManager = function (
    canvas,
    getVertexShader,
    getFragmentShader,
    onRestore,
            ...
            (function (obj, onRestore) {
              ...
              canvas.addEventListener(
                ...
                (ev) => {
                  ev.preventDefault();
                  if (debug)
                    console.debug("WebGL context restored at " + Date());
                  obj.restored = true;
  ...
  DRAW.GL.getContextSimulator = function (id) {
    if (!id) {
      ...
      console.log(
        "Valid ids are like summary_canvas, row_class_canvas, col_class_canvas, detail_canvas, detail_canvas2, etc.",
      );
      return;
    }
    return document

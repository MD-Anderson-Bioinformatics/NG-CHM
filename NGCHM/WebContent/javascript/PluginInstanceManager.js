...
(function () {
  ...
  class PluginInstance {
    constructor({
      ...
      if (
        ["panel-plugin", "linkout-plugin", "hamburger-plugin"].indexOf(kind) ===
        -1
      ) {
        alert("Unknown plugin kind");
      }
      Object.assign(this, {
        kind,
        nonce,
  ...
  PIM.removePluginInstance = function removePluginInstance(loc, elements) {
    const nonceToRemove =
      elements[0].getElementsByTagName("IFRAME")[0].dataset.nonce;
  ...
  PIM.setPanePluginOptions = function (element, options, initializePlugin) {
    ...
    loc.paneTitle.innerText =
      instances[nonce].plugin.name + ". " + options.plotTitle;
  ...
  PIM.createPluginInstance = function createPluginInstance(kind, plugin) {
    ...
    const url = isBlob
      ? plugin.src
      : plugin.src +
        (plugin.src.indexOf("?") == -1 ? "?" : "&") +
        "nonce=" +
        nonce;

    const iframe = document.createElement("IFRAME");
    iframe.dataset.nonce = nonce;
    instances[nonce] = new PluginInstance({
      kind,
      nonce,
      plugin,
      params: {},
  ...
  PIM.sendMessageToPlugin = function sendMessageToPlugin(msg) {
    const src =
      instances[msg.nonce].source || instances[msg.nonce].iframe.contentWindow;
    if (src !== null) {
  ...
  PIM.havePluginData = function havePluginData() {
    ...
    if (
      JSON.stringify(havePluginDataList.sort()) ===
      JSON.stringify(Object.keys(pluginInstances).sort())
    ) {
      return true;
    } else {
      return false;
  ...
  PIM.warnAboutMissingPluginData = function warnAboutMissingPluginData() {
    ...
    let warningText =
      "Unable to save some data elements from the following plugins: <br>";
    let pluginInstances = PIM.getPluginInstances();
    Object.keys(pluginInstances).forEach((pi) => {
      if (typeof pluginInstances[pi]["dataFromPlugin"] == "undefined") {
        warningText +=
          "<br>&nbsp;&nbsp;" + pluginInstances[pi]["plugin"]["name"];
      }
    });
  ...
  PIM.sendMessageToAllOtherPlugins = function sendMessageToAllOtherPlugins(
    srcNonce,
    msg,
  ) {

...
(function() {
    ...
    function addTransform (typename) {
        if (CUST.verbose) console.log( 'NgChm.CUST: adding transforms for ' + typename );
        ...
        if (ff.length > 1) {
		    ...
		    for (var fidx = 1; fidx < ff.length; fidx++) {
		        if (ff[fidx].indexOf("list.") === 0) {
					...
					if (CUST.verbose) console.log( 'Error: badly formed type modifier ' + ff[idx] );
		        	}
		        } else {
				if (CUST.verbose) console.log( 'Error: unknown type modifier ' + ff[idx] );
		        }
		    }
            CUST.transforms.push({ baseType: ff[0], typeName: typename, doTypeTransform: doTypeTransform, xforms: xforms.reverse() });
        }
    }
    ...
    function doTypeTransform (transform, menuEntry, linkoutType, selectMode, linkoutFn, attributes) {
        ...
        if (linkoutType === transform.baseType || supertypes.indexOf(linkoutType)>=0) {
            if (CUST.verbose) console.log( 'Adding typetransform linkout "' + menuEntry + '" for type ' + transform.typeName + ' mode ' + selectMode );
	...
	var subTypes = [subtype].concat(CUST.subTypes[subtype] || []).sort();
	    ...
	    if (!CUST.subTypes.hasOwnProperty(supertype)) CUST.subTypes[supertype] = [];
            for (bidx = 0; bidx < subTypes.length; bidx++) {
                subtype = subTypes[bidx];
	        if (!CUST.superTypes.hasOwnProperty(subtype)) CUST.superTypes[subtype] = [];
	...
	if (CUST.verbose) console.log ('NgChm.CUST.createPluginLinkout "' + menuEntry + '" typeName: ' + typeName + ' selectMode: ' + selectMode );
	...
	if (CUST.allTypes.indexOf( typeName ) >= 0) {
	    if (CUST.verbose) console.log( 'Adding type linkout for "' + menuEntry + '" typeName: ' + typeName + ' selectMode: ' + selectMode );
	...
	subtypes.forEach(function(subtype) {
		...
		if (CUST.verbose) console.log( 'Adding subtype linkout for "' + menuEntry + '" typeName: ' + subtype + ' selectMode: ' + selectMode );
	...
	for (var pidx = 0; pidx < CUST.customPlugins.length; pidx++) {
	    var plugin = CUST.customPlugins[pidx];
	    if (plugin.linkouts) {
		if (CUST.verbose) console.log( 'Defining custom linkouts for plugin ' + plugin.name );
		...
		if (CUST.verbose) console.log( 'Defining custom matrix linkouts for plugin ' + plugin.name );

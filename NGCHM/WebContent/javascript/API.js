(function () {
  ...
  function getSummaryHist(thresholds) {
    ...
    return heatMap.getSummaryHist(
      heatMap.getCurrentDL(),
      +thresholds[0],
  ...
  function combinePngImage(img1, img2, img3, width, height, dl, callback) {
    ...
    var cDShow =
      colDConfShow === "NONE" || colDConfShow === "NA" ? false : true;
    var rDShow =
      rowDConfShow === "NONE" || rowDConfShow === "NA" ? false : true;
    var mapWidth = width;
  ...
  function scalePngImage(origCanvas, width, height, dl, callback) {
    ...
    if (url.length < 10) {
      UHM.systemMessage(
        "Download Thumbnail Warning",
        "The Summary Pane must be open and visible in the NG-CHM Viewer in order to download a thumbnail image of the heat map.",
      );
      return;
    }

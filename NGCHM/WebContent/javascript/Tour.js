...
(function() {
	...
	if (loc) {
	    const steps = [...loc.pane.querySelectorAll("*[data-intro]")].filter(isVisible).map (e => {
	...
	const tourElements = querySelectors.map(qs => document.querySelector(qs)).filter (isVisible);
	const steps = tourElements.map(e => {
	    const step = { element: e, intro: e.dataset.intro || "Missing data-intro" };
	    if (e.dataset.title) step.title = e.dataset.title;
	    return step;
	});
	const infoDetails = "<P>Also, our <A href='https://www.youtube.com/channel/UCADGir2q8IaI9cGQuzjSL9w/playlists' target='_blank'>YouTube channel</A> has a <A href='https://www.youtube.com/playlist?list=PLIBaINv-Qmd05G3Kj7SbBbSAPZrG-H5bq' target='_blank'>playlist of short videos</A> describing and demonstrating key user interface elements.</P>"
	steps.unshift ({ title: "Brief Interface Tour", intro: "We'll briefly highlight and describe the major features of the NG-CHM viewer that you can currently see." + infoDetails});
	if (true) {
	    // Add help items for dynamicly inserted panels.
	    let e = document.querySelector (".pane");
	    if (e && e.clientHeight > 0) {
		steps.push ({ element: e, title: "Panel", intro: "<P>Content panels display NG-CHM visualizations, such as summary views, detail views, or plugin components.</P><P>A panel-specific tour may be available from the panel menu button on the right of the panel header.</P>" });
	...
	steps.push ({ title: "End of Tour", intro: "We have briefly highlighted and described the major features of the NG-CHM viewer." + infoDetails });
	introJs().setOptions({steps}).start();
    }
	...
	if (el.id == 'closeSplash') return isVisible (document.getElementById ('splash'));
	...
	if (chm && chm.clientHeight > 0) {
	    steps.push ({ element: chm, title: "Detail (Zoomed) View", intro: "Displays a detailed (or zoomed) view of part of the map. Clicking on the detail view will display information about the data point(s) at that position. You can also drag the detail view to pan the display." });

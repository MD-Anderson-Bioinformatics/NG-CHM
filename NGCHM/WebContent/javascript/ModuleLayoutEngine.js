class ModuleLayoutEngine {
    ...
    this.each(this.graph.nodes, (node) => {
      ...
      if (node.trace)
        console.log({
          ...
          id: node.id,
          c: this.c,
          fx: node.layoutForceX,
          xmove,
          fy: node.layoutForceY,
          ymove,
          node,
        });
  ...
  layoutRepulsive(node1, node2) {
    ...
    if (d < 10 * this.maxRepulsiveForceDistance) {
      const repulsiveForceX =
        (this.k * this.k) / Math.max(d, this.maxRepulsiveForceDistance);
      let repulsiveForceY = repulsiveForceX;
      if (node1.level > node2.level && node1.layoutPosY <= node2.layoutPosY) {
        repulsiveForceY = 0;
      }
      if (node1.trace && node2.trace)
        console.log({
          m: "Node trace",
          node1: node1.id,
          node2: node2.id,
          ...
          repulsiveForceY,
          d,
          maxD: this.maxRepulsiveForceDistance,
        });
  ...
  layoutAttractive(edge) {
    ...
    if (edge.op == "export") attractiveForceX *= 2;
    else attractiveForceX *= 0.005;

    if (edge.directed) {
      if (node2.level <= node1.level)
        node2.level = node1.level + (edge.op == "export" ? 1 : 2);
    }

...
(function() {
	...
	if (scriptLoaded) {
	    ...
	    dynload (['javascript/lib/raphael.js', 'javascript/lib/dracula.min.js', 'javascript/ModuleLayoutEngine.js'], () => {
		drawGraph();
	    });
	}
	...
	function drawGraph(script) {
		...
		uniqEdges.push(key);
		if (entry.op == 'export') {
		    g.addEdge (entry.ns, entry.src, { directed: true, color: '#aaaaff', op: entry.op });
		} else {
		    g.addEdge (entry.src, entry.ns, { directed: true, color: '#ffaaaa', op: entry.op });
		}
	    });
		...
		const uniqSrcX = [];
		sourceX.forEach(x => { if (uniqSrcX.indexOf(x) == -1) uniqSrcX.push(x); });
		const newSourceX = [];
		newSourceX.length = sourceX.length;
		uniqSrcX.forEach(ux => {
		    let posn = 0;
		    const details = sourceX.map((x,index) => ({
			x: x,
			index: index,
			...
			for (let i = 0; i < N; i++) {
				...
				let leftidx = details.map((det,idx) => [det,idx]).filter(di => di[0].posn == posn-1);
				leftidx = leftidx[0];
				leftidx = leftidx[1];
				if (details[i].targetY < details[leftidx].targetY ||
					(details[i].targetY == details[leftidx].targetY && details[i].targetX < details[leftidx].targetX)) {
				...
				let rightidx = details.map((det,idx) => [det,idx]).filter(di => di[0].posn == posn+1)[0][1];
				if (details[i].targetY < details[rightidx].targetY ||
					(details[i].targetY == details[rightidx].targetY && details[i].targetX >= details[rightidx].targetX)) {
				    let tmp = details[i].posn;
		    ...
		    details.forEach(det => { newSourceX[det.index] = ux + (det.posn + 1) * dX; });
		});
		...
		const uniqTgtX = [];
		targetX.forEach(x => { if (uniqTgtX.indexOf(x) == -1) uniqTgtX.push(x); });
		const newTargetX = [];
		newTargetX.length = sourceX.length;
		uniqTgtX.forEach(ux => {
		    const details = targetX.map((x,index) => ({
			x: x,
			index: index,
		    ...
		    const dX = details[0].width / (N + 1);
		    details.forEach((det,idx) => { newTargetX[det.index] = ux + (idx + 1) * dX; });
		});
		...
		const barY = [];
		const uniqY = [];
		targetY.forEach(y => { if (uniqY.indexOf(y) == -1) uniqY.push(y); });
		barY.length = targetY.length;
		uniqY.forEach(uy => {
		    const details = targetY.map((y, index) => {
			let det = { y: y, index: index, sourceY: sourceY[index], sourceX: sourceX[index], targetX: newTargetX[index] };
			det.minX = det.sourceX < det.targetX ? det.sourceX : det.targetX;
			det.maxX = det.sourceX > det.targetX ? det.sourceX : det.targetX;
			return det;
		    }).filter(det => det.y == uy);
		    const N = details.length;
		    details.sort ((a, b) => { if (a.minX < b.minX) return -1; if (b.minX < a.minX) return 1; return 0; });
			...
			group.sort ((a, b) => Math.sign (a.sourceX - b.sourceX) * Math.sign (b.sourceY - a.sourceY));
			// Find maximum y of ALL nodes within the x limits of the group and above the current y.
			let maxSourceY = 0;
			Object.entries(layout.graph.nodes).forEach(([key,node]) => {
			    const x = xt(node.layoutPosX);
			    const y = yt(node.layoutPosY);
			    if (y < uy && x >= group[0].minX && x <= grpEndX && y > maxSourceY) {
				maxSourceY = y;
			    }
			});
			...
			group.forEach(det => { barY[det.index] = nextY; nextY += dY; });
		...
		Object.entries(layout.graph.edges).forEach(([key,edge]) => {
		    ...
		    if (ty > sy+boxHeight) { sy += boxHeight; }
		    else if (sy > ty+boxHeight) { ty += boxHeight; }
		    else { sy += 15; ty += 15; }

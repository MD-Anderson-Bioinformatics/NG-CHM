(function () {
  ...
  DET.zoomBoxSizes = [
    1, 2, 3, 4, 6, 7, 8, 9, 12, 14, 18, 21, 24, 28, 36, 42, 56, 63, 72, 84, 126,
    168, 252,
  ];
  ...
  DET.setInitialDetailDisplaySize = function (mapItem) {
    ...
    if (rows < 42 || cols < 42) {
      const boxSize = DET.getNearestBoxSize(
        mapItem,
        "column",
        Math.min(rows, cols),
      );
  ...
  DET.callDetailDrawFunction = function (modeVal, target) {
    ...
    if (modeVal == "FULL_MAP") DET.detailFullMap(mapItem);
    if (modeVal == "NORMAL") {
  ...
  DET.drawDetailHeatMap = function (mapItem, drawWin) {
    ...
    const colorMap = heatMap
      .getColorMapManager()
      .getColorMap("data", drawWin.layer);
    ...
    // Set parameters that depend on the data layer properties.
    {
      ...
      params.showVerticalGrid =
        showGrid &&
        params.dataBoxWidth > UTIL.minLabelSize &&
        DET.minPixelsForGrid * drawWin.numCols <= detWidth;
      params.showHorizontalGrid =
        showGrid &&
        params.dataBoxHeight > UTIL.minLabelSize &&
        DET.minPixelsForGrid * drawWin.numRows <= detHeight;
  ...
  DET.getDetailHeatMap = function (mapItem, drawWin, params) {
    ...
    if (mapItem.detailHeatMapCache.hasOwnProperty(layer)) {
      ...
      mapItem.detailHeatMapCache[layer] = DRAW.createRenderBuffer(
        texWidth,
        texHeight,
        1.0,
      );
    ...
    const colorMap = heatMap
      .getColorMapManager()
      .getColorMap("data", layer)
      .getContColorMap();

    const dataGridColor = CMM.hexToRgba(params.grid_color);
    const dataSelectionColorRGB = CMM.hexToRgba(params.selection_color);
    const dataSelectionColor = [
      dataSelectionColorRGB.r / 255,
      dataSelectionColorRGB.g / 255,
      dataSelectionColorRGB.b / 255,
      1,
    ];
    const regularGridColor = [
      dataGridColor.r,
      dataGridColor.g,
      dataGridColor.b,
    ];
    ...
    if (cutsColor !== null && cutsColor !== undefined) {
      for (let i = 0; i < linelen; i++) {
        cutsLine[i] = cutsColor.r;
        i++;
        cutsLine[i] = cutsColor.g;
        i++;
        cutsLine[i] = cutsColor.b;
        i++;
    ...
    if (params.showHorizontalGrid) {
      ...
      for (let j = 0; j < detDataPerRow; j++) {
        ...
        const gridColor =
          params.searchCols.indexOf(mapItem.currentCol + j) > -1 ||
          params.searchCols.indexOf(mapItem.currentCol + j + 1) > -1
            ? params.searchGridColor
            : regularGridColor;
        for (let k = 0; k < mapItem.dataBoxWidth; k++) {
          ...
          if (val <= MAPREP.minValues) {
            if (k === mapItem.dataBoxWidth - 1 && nextVal > MAPREP.minValues) {
              gridLine[linePos] = gridColor[0];
              gridLine[linePos + 1] = gridColor[1];
              gridLine[linePos + 2] = gridColor[2];
              gridLine[linePos + 3] = 255;
            } else {
              gridLine[linePos] = cutsColor.r;
              gridLine[linePos + 1] = cutsColor.g;
              gridLine[linePos + 2] = cutsColor.b;
              gridLine[linePos + 3] = cutsColor.a;
            }
          } else {
            if (k == mapItem.dataBoxWidth - 1) {
              // should the grid line be drawn?
              gridLine[linePos] = gridColor[0];
              gridLine[linePos + 1] = gridColor[1];
              gridLine[linePos + 2] = gridColor[2];
              gridLine[linePos + 3] = 255;
            } else {
              gridLine[linePos] = regularGridColor[0];
              gridLine[linePos + 1] = regularGridColor[1];
              gridLine[linePos + 2] = regularGridColor[2];
              gridLine[linePos + 3] = 255;
            }
          }
          linePos += DRAW.BYTE_PER_RGBA;
    ...
    let line = new Uint8Array(
      new ArrayBuffer(
        (rowClassBarWidth + mapItem.dataViewWidth) * DRAW.BYTE_PER_RGBA,
      ),
    );
    ...
    for (let i = detDataPerCol - 1; i >= 0; i--) {
      ...
      const isHorizCut =
        isLineACut(accessWindow, i) && isLineACut(accessWindow, i - 1);
      linePos += DRAW.BYTE_PER_RGBA;
      // Get value generator for this row.
      const valueGen = accessWindow
        .getRowValues(currDetRow + i)
        [Symbol.iterator]();
      ...
      for (let j = 0; j < detDataPerRow; j++) {
        ...
        if (val === undefined) {
          console.error("accessWindow value generator returned undefined", {
            accessWindow,
            row: currDetRow + i,
            col: currDetCol + j,
            detDataPerRow,
            ...
            if (
              ...
              if (j < detDataPerRow - 1) {
                ...
                if (val <= MAPREP.minValues && nextVal <= MAPREP.minValues) {
                  line[linePos] = cutsColor.r;
                  line[linePos + 1] = cutsColor.g;
                  line[linePos + 2] = cutsColor.b;
                  line[linePos + 3] = cutsColor.a;
                } else {
                  line[linePos] = regularGridColor[0];
                  line[linePos + 1] = regularGridColor[1];
                  line[linePos + 2] = regularGridColor[2];
                  line[linePos + 3] = 255;
                }
              }
            } else {
  ...
  function isLineACut(accessWindow, row) {
    ...
    const resultsMemo =
      accessWindow.heatMap.datalevels[accessWindow.win.level].isLineACut;
  ...
  DET.getNearestBoxSize = function (mapItem, axis, sizeToGet) {
    const smallestMapDimension = Math.min(
      mapItem.heatMap.getTotalRows(),
  ...
  DET.setDetailDataWidth = function (mapItem, size) {
    ...
    DVW.setDataPerRowFromDet(
      Math.floor(
        (mapItem.dataViewWidth - DET.dataViewBorder) / mapItem.dataBoxWidth,
      ),
      mapItem,
    );
    ...
    if (prevDataPerRow != null && mapItem.modeHistory.length === 0) {
      if (prevDataPerRow > mapItem.dataPerRow) {
        mapItem.currentCol += Math.floor(
          (prevDataPerRow - mapItem.dataPerRow) / 2,
        );
      } else {
        mapItem.currentCol -= Math.floor(
          (mapItem.dataPerRow - prevDataPerRow) / 2,
        );
      }
    }
  ...
  DET.setDetailDataHeight = function (mapItem, size) {
    ...
    DVW.setDataPerColFromDet(
      Math.floor(
        (mapItem.dataViewHeight - DET.dataViewBorder) / mapItem.dataBoxHeight,
      ),
      mapItem,
    );
    if (!mapItem.modeHistory) mapItem.modeHistory = [];
    ...
    if (prevDataPerCol != null && mapItem.modeHistory.length === 0) {
      if (prevDataPerCol > mapItem.dataPerCol)
        mapItem.currentRow += Math.floor(
          (prevDataPerCol - mapItem.dataPerCol) / 2,
        );
      else
        mapItem.currentRow -= Math.floor(
          (mapItem.dataPerCol - prevDataPerCol) / 2,
        );
    }
  ...
  DET.detailFullMap = function (mapItem) {
    ...
    if (mapItem.mode.startsWith("RIBBONH") && mapItem.selectedStart != 0) {
      // mapItem.subDendroMode === 'Row') {
      if (
        mapItem.currentRow == 1 &&
        mapItem.dataPerCol == mapItem.heatMap.getTotalRows()
      ) {
        setFullMap = true;
      } else {
      ...
      // subDendroMode === 'Column') {
      if (
        mapItem.currentCol == 1 &&
        mapItem.dataPerRow == mapItem.heatMap.getTotalCols()
      ) {
        setFullMap = true;
      } else {
  ...
  function setCanvasDimensions(mapItem) {
    mapItem.canvas.width =
      ...
      mapItem.getScaledVisibleCovariates("row").totalHeight();
    mapItem.canvas.height =
      mapItem.dataViewHeight +
      mapItem.getScaledVisibleCovariates("column").totalHeight();
  }
  ...
  DET.detailHRibbon = function (mapItem, restoreInfo) {
    ...
    if (mapItem.selectedStart == null || mapItem.selectedStart == 0) {
      const numRibbonColumns = mapItem.heatMap.getNumColumns(
        MAPREP.RIBBON_HOR_LEVEL,
      );
      let ddw = 1;
      while (ddw * numRibbonColumns < 250 - DET.dataViewBorder) {
        // make the width wider to prevent blurry/big dendros for smaller maps
        ddw += ddw;
      }
      setDataViewSize(
        mapItem,
        "row",
        ddw * numRibbonColumns + DET.dataViewBorder,
      );
      ...
      setDataViewSize(
        mapItem,
        "row",
        numViewColumns * scale + DET.dataViewBorder,
      );
      DET.setDetailDataWidth(mapItem, scale);
    ...
    if (!restoreInfo) {
      ...
      if (
        previousMode == "RIBBONV" ||
        previousMode == "RIBBONV_DETAIL" ||
        previousMode == "FULL_MAP"
      ) {
        if (previousMode == "FULL_MAP") {
          DET.setDetailDataHeight(mapItem, DET.zoomBoxSizes[0]);
        } else {
      ...
      while (
        Math.floor(
          dataViewSize /
            DET.zoomBoxSizes[DET.zoomBoxSizes.indexOf(mapItem.dataBoxHeight)],
        ) > numDetailRows
      ) {
        DET.setDetailDataHeight(
          mapItem,
          DET.zoomBoxSizes[DET.zoomBoxSizes.indexOf(mapItem.dataBoxHeight) + 1],
        );
      }
    }
  ...
  DET.detailVRibbon = function (mapItem, restoreInfo) {
    ...
    // and data size to 1.
    if (mapItem.selectedStart == null || mapItem.selectedStart == 0) {
      const numRibbonRows = mapItem.heatMap.getNumRows(
        MAPREP.RIBBON_VERT_LEVEL,
      );
      let ddh = 1;
      while (ddh * numRibbonRows < 250 - DET.dataViewBorder) {
        // make the height taller to prevent blurry/big dendros for smaller maps
        ddh += ddh;
      }
      DET.setDataViewSize(
        mapItem,
        "column",
        ddh * numRibbonRows + DET.dataViewBorder,
      );
      ...
      if (selectionSize < 500) {
        ...
        const rvRate = mapItem.heatMap.getRowSummaryRatio(
          MAPREP.RIBBON_VERT_LEVEL,
        );
      ...
      setDataViewSize(
        mapItem,
        "column",
        selectionSize * height + DET.dataViewBorder,
      );
    ...
    if (!restoreInfo) {
      ...
      if (
        previousMode == "RIBBONH" ||
        previousMode == "RIBBONH_DETAIL" ||
        previousMode == "FULL_MAP"
      ) {
        if (previousMode == "FULL_MAP") {
      ...
      while (
        Math.floor(
          (mapItem.dataViewWidth - DET.dataViewBorder) /
            DET.zoomBoxSizes[DET.zoomBoxSizes.indexOf(mapItem.dataBoxWidth)],
        ) > numDetailColumns
      ) {
        DET.setDetailDataWidth(
          mapItem,
          DET.zoomBoxSizes[DET.zoomBoxSizes.indexOf(mapItem.dataBoxWidth) + 1],
        );
      }
    }
  ...
  DET.restoreFromSavedState = function (mapItem, savedState) {
    ...
    DET.setDataViewSize(
      mapItem,
      "row",
      savedState.dataViewWidth + DET.dataViewBorder,
    );
    DET.setDataViewSize(
      mapItem,
      "column",
      savedState.dataViewHeight + DET.dataViewBorder,
    );
  ...
  (function () {
    ...
    function drawMapItemSelectionsOnTarget(mapItem, target) {
      ...
      if (rowRanges.length > 0 || colRanges.length > 0) {
        ...
        mapItemVars.ctx.lineWidth =
          3 * Math.min(target.widthScale, target.heightScale);
        ...
        if (rowRanges.length === 0) {
          ...
          calcVisColRanges(colRanges, target.widthScale, mapItem).forEach(
            ([left, right]) => {
          ...
          calcVisRowRanges(rowRanges, target.heightScale, mapItem).forEach(
            ([topY, bottom]) => {
              drawSearchBox(mapItem, topY, bottom, left, right);
            },
          );
          ...
          const visColRanges = calcVisColRanges(
            colRanges,
            target.widthScale,
            mapItem,
          );
      ...
      if (debug) {
        const elapsedTime =
          Math.round(10 * (performance.now() - mapItemVars.start)) / 10;
        console.log(
          "Detail map ",
          k + 1,
          ": Drew ",
    ...
    function calcMapItemVariables(mapItem, target) {
      ...
      mapItemVars.totalRowBarHeight = mapItem
        .getScaledVisibleCovariates("row")
        .totalHeight();
      mapItemVars.totalColBarHeight = mapItem
      ...
      mapItemVars.topX =
        (mapItemVars.totalRowBarHeight / mapItem.canvas.width) * target.width;
      mapItemVars.topY =
        (mapItemVars.totalColBarHeight / mapItem.canvas.height) * target.height;
      ...
      if (mapItem.mode === "NORMAL" || mapItem.mode === "RIBBONV") {
        mapItemVars.cellWidth =
          mapItemVars.mapXWidth /
          (DVW.getCurrentDetDataPerRow(mapItem) * target.widthScale);
      } else {
        mapItemVars.cellWidth =
          mapItemVars.mapXWidth / (mapItem.dataPerRow * target.widthScale);
      }
      ...
      if (mapItem.mode === "NORMAL" || mapItem.mode === "RIBBONH") {
        mapItemVars.cellHeight =
          mapItemVars.mapYHeight /
          (DVW.getCurrentDetDataPerCol(mapItem) * target.heightScale);
      } else {
        mapItemVars.cellHeight =
          mapItemVars.mapYHeight / (mapItem.dataPerCol * target.heightScale);
      }
    ...
    function calcVisRanges(
      ...
      if (debug)
        console.log("Found ", visRanges.length, " visible ", axis, " ranges");
      return visRanges;
    }
    ...
    function calcVisColRanges(ranges, widthScale, mapItem) {
      return calcVisRanges(
        "column",
        ranges,
        mapItem.currentCol,
        widthScale,
        mapItemVars.topX,
        mapItemVars.target.width,
        mapItemVars.cellWidth,
      );
    }
  ...
  function sizeCanvasForLabels(mapItem) {
    ...
    if (mapItem.pane !== "") {
      //Used by builder which does not contain the detail pane necessary, nor the use, for this logic
  ...
  function setViewPort(mapItem) {
    ...
    UTIL.setElementPositionSize(
      document.getElementById(mapItem.rowLabelDiv),
      rowLabelVP,
      true,
    );

    const heightCalc =
      dFullH - mapItem.canvas.offsetTop - mapItem.canvas.offsetHeight;
    const colLabelVP = {
    ...
    UTIL.setElementPositionSize(
      document.getElementById(mapItem.colLabelDiv),
      colLabelVP,
      true,
    );
  }
  ...
  function calcRowAndColLabels(mapItem) {
    ...
    let fontSize;
    if (
      mapItem.rowLabelFont >= UTIL.minLabelSize &&
      mapItem.colLabelFont >= UTIL.minLabelSize
    ) {
  ...
  function calcCovariateBarLabelFont(mapItem, axis, bars) {
    ...
    if (MAPREP.isRow(axis)) {
      ...
      scale =
        ...
        (mapItem.dataViewHeight + bars.totalHeight());
    }
    ...
    bars.forEach((classBar) => {
      const fontSize = Math.min(
        (classBar.height - DET.paddingHeight) * scale - 1,
        10,
      );
  ...
  function calcAxisLabelFontSize(mapItem, axis) {
    ...
    const targetSize = Math.min(
      DET.maxLabelSize,
      Math.max(UTIL.minLabelSize, skip),
    );
  ...
  function drawAxisLabels(mapItem, axis, fontSize) {
    ...
    if (skip > UTIL.minLabelSize) {
      ...
      for (let i = 0; i < numLabels; i++) {
        ...
        if (actualLabels[idx - 1] !== undefined) {
          // an occasional problem in subdendro view
          DET.addLabelDiv(
            /* mapItem   */ mapItem,
  ...
  DET.detailDrawRowClassBarLabels = function (mapItem) {
    ...
    if (mapItem.rowClassLabelFont > UTIL.minLabelSize) {
      ...
      if (rowBars.length > 0) {
        ...
        const scale =
          mapItem.canvas.clientWidth /
          (mapItem.dataViewWidth + rowBars.totalHeight());
        ...
        let startingPoint =
          mapItem.canvas.offsetLeft + mapItem.rowClassLabelFont + 2;
        rowBars.forEach((currentClassBar, index) => {
          ...
          const availableSpace = Math.min(
            (currentClassBar.height - DET.paddingHeight) * scale,
            DET.maxLabelSize,
          );
          if (availableSpace >= mapItem.rowClassLabelFont) {
            ...
            DET.addLabelDiv(
              ...
              "detail_classrow" + index + mapItem.labelPostScript,
              "DynamicLabel ClassBar",
              labelText,
              currentClassBar.label,
              xPos,
              yPos,
              mapItem.rowClassLabelFont,
              "T",
              currentClassBar.idx,
              "RowCovar",
              currentClassBar.label,
    ...
    if (mapItem.heatMap.hasHiddenCovariates("row")) {
      if (!document.getElementById("missingDetRowClassBars")) {
        ...
        DET.addLabelDiv(
          mapItem,
          mapItem.labelElement,
          "missingDetRowClassBars" + mapItem.labelPostScript,
          "ClassBar MarkLabel",
          "...",
          "...",
          x,
      ...
      if (!document.getElementById("missingSumRowClassBars") && SUM.canvas) {
        ...
        if (sumlabelDiv !== null) {
          DET.addLabelDiv(
            ...
            "missingSumRowClassBars" + mapItem.labelPostScript,
            "ClassBar MarkLabel",
            "...",
            "...",
            x,
  ...
  DET.detailDrawColClassBarLabels = function (mapItem) {
    ...
    if (mapItem.colClassLabelFont > UTIL.minLabelSize) {
      ...
      const scale =
        mapItem.canvas.clientHeight /
        (mapItem.dataViewHeight + colBars.totalHeight());
      if (colBars.length > 0) {
        ...
        colBars.forEach((currentClassBar, index) => {
          ...
          if (spaceAvailable >= mapItem.colClassLabelFont) {
            ...
            DET.addLabelDiv(
              mapItem,
              mapItem.labelElement,
              "detail_classcol" + index + mapItem.labelPostScript,
              "DynamicLabel ClassBar",
              labelText,
              currentClassBar.label,
              xPos,
              yPos + delta,
              mapItem.colClassLabelFont,
              "F",
              currentClassBar.idx,
              "ColumnCovar",
            );
    ...
    if (mapItem.heatMap.hasHiddenCovariates("column")) {
      if (!document.getElementById("missingDetColClassBars")) {
        ...
        DET.addLabelDiv(
          mapItem,
          mapItem.labelElement,
          "missingDetColClassBars" + mapItem.labelPostScript,
          "ClassBar MarkLabel",
          "...",
          "...",
          x,
      ...
      if (
        !document.getElementById("missingSumColClassBars") &&
        SUM.canvas &&
        SUM.chmElement
      ) {
        ...
        DET.addLabelDiv(
          mapItem,
          document.getElementById("sumlabelDiv"),
          "missingSumColClassBars" + mapItem.labelPostScript,
          "ClassBar MarkLabel",
          "...",
          "...",
          x,
   ...
   ************************************************************************************************/
  DET.bullet = "* ";
  DET.drawRowClassBarLegend = function (
    mapItem,
    currentClassBar,
    barStartPosn,
    totalHeight,
    ...
    const scale =
      mapItem.canvas.clientWidth / (mapItem.dataViewWidth + totalHeight);
    ...
    const digits = Math.min(
      3,
      Math.max(0, Math.ceil(1 - Math.log10(highVal - lowVal))),
    );
    ...
    DET.setLegendDivElement(
      mapItem,
      currentClassBar.label + mapItem.panelNbr + "legendDetLow",
      DET.bullet + lowVal,
      topPos,
      beginPos,
      true,
    ...
    DET.setLegendDivElement(
      mapItem,
      currentClassBar.label + mapItem.panelNbr + "legendDetMid",
      DET.bullet + midVal,
      topPos,
      midPos,
      true,
    ...
    DET.setLegendDivElement(
      mapItem,
      currentClassBar.label + mapItem.panelNbr + "legendDetHigh",
      DET.bullet + highVal,
      topPos,
      endPos,
      true,
   ...
   * detail panel for maps that contain bar/scatter plot covariates.
   ************************************************************************************************/
  DET.drawColClassBarLegend = function (
    mapItem,
    currentClassBar,
    startPosn,
    totalHeight,
    ...
    const scale =
      mapItem.canvas.clientHeight / (mapItem.dataViewHeight + totalHeight);
  ...
  DET.removeClassBarLegendElements = function (key, mapItem) {
    let legItem = document.getElementById(
      key + mapItem.panelNbr + "legendDetHigh-",
    );
   ...
   ************************************************************************************************/
  DET.setLegendDivElement = function (
    mapItem,
    itemId,
    boundVal,
    topVal,
  ...
  DET.addLabelDiv = function (
    ...
    if (text !== "<" && text !== "..." && text.length > 0) {
      div.addEventListener(
        "click",
        mapItem.labelCallbacks.labelClick,
        UTIL.passiveCompat({ capture: false, passive: false }),
      );
      div.addEventListener(
        "contextmenu",
        mapItem.labelCallbacks.labelRightClick,
        UTIL.passiveCompat({ capture: false, passive: false }),
      );
      div.onmouseover = function () {
        UHM.hlp(this, longText, longText.length * 9, 0);
      };
      ...
      div.addEventListener(
        "touchmove",
        mapItem.labelCallbacks.labelDrag,
        UTIL.passiveCompat({ passive: false }),
      );
    }
    if (text == "...") {
      ...
      div.addEventListener(
        ...
        (function () {
          return function (e) {
            UHM.hlp(this, "Some covariate bars are hidden", 160, 0);
          };
        })(this),
        listenOpts,
      );
  ...
  DET.setDendroShow = function (mapItem) {
    ...
    if (!heatMap.showRowDendrogram("DETAIL")) {
      ...
      mapItem.dendroWidth = Math.floor(
        (parseInt(rowDendroConfig.height) *
          heatMap.getRowDendroConfig().height) /
          100 +
          5,
      );
    }
    if (!heatMap.showColDendrogram("DETAIL")) {
      ...
      mapItem.dendroHeight = Math.floor(
        (parseInt(colDendroConfig.height) *
          heatMap.getColDendroConfig().height) /
          100 +
          5,
      );
    }
  ...
  DET.colDendroResize = function (mapItem, drawIt) {
    if (mapItem.colDendroCanvas !== null) {
      ...
      const canW =
        mapItem.dataViewWidth +
        mapItem.getScaledVisibleCovariates("row").totalHeight();
      dendroCanvas.style.left =
        left +
        mapItem.canvas.clientWidth * (1 - mapItem.dataViewWidth / canW) +
        "px";
      if (mapItem.colDendro.isVisible()) {
        ...
        dendroCanvas.style.width =
          mapItem.canvas.clientWidth * (mapItem.dataViewWidth / canW) + "px";
        mapItem.colDendro.setCanvasSize(
          Math.round(
            mapItem.canvas.clientWidth *
              (mapItem.dataViewWidth / mapItem.canvas.width),
          ),
          Math.round(height),
        );
  ...
  DET.rowDendroResize = function (mapItem, drawIt) {
    if (mapItem.rowDendroCanvas !== null) {
      ...
      const canH =
        mapItem.dataViewHeight +
        mapItem.getScaledVisibleCovariates("column").totalHeight();
      dendroCanvas.style.top =
        top +
        mapItem.canvas.clientHeight * (1 - mapItem.dataViewHeight / canH) +
        "px";
      if (mapItem.rowDendro.isVisible()) {
        ...
        const height =
          mapItem.canvas.clientHeight * (mapItem.dataViewHeight / canH);
  ...
  DET.getColDendroPixelHeight = function (mapItem) {
    return (
      mapItem.canvas.clientHeight *
      (mapItem.dendroHeight / mapItem.canvas.height)
    );
  };
  ...
  DET.getRowDendroPixelWidth = function (mapItem) {
    return (
      mapItem.canvas.clientWidth * (mapItem.dendroWidth / mapItem.canvas.width)
    );
  };
  ...
  DET.getColClassPixelHeight = function (mapItem) {
    const classbarHeight = mapItem
      .getScaledVisibleCovariates("column")
      .totalHeight();
    return (
      mapItem.canvas.clientHeight * (classbarHeight / mapItem.canvas.height)
    );
  };
  ...
  DET.detailDrawColClassBars = function (mapItem, pixels) {
    ...
    const rowClassBarWidth = mapItem
      .getScaledVisibleCovariates("row")
      .totalHeight();
    ...
    bars.reverse().forEach((currentClassBar) => {
      ...
      if (
        (mapItem.mode == "RIBBONH" || mapItem.mode == "FULL_MAP") &&
        typeof colClassBarData[currentClassBar.label].svalues !== "undefined"
      ) {
      ...
      if (currentClassBar.bar_type === "color_plot") {
        pos = DET.drawColorPlotColClassBar(
          mapItem,
          pixels,
          pos,
          rowClassBarWidth,
        ...
        pos = DET.drawScatterBarPlotColClassBar(
          mapItem,
          pixels,
          pos,
          rowClassBarWidth,
   ...
   **********************************************************************************/
  DET.drawColorPlotColClassBar = function (
    mapItem,
    pixels,
    pos,
    rowClassBarWidth,
    ...
    const line = new Uint8Array(
      new ArrayBuffer(barLength * DRAW.BYTE_PER_RGBA),
    ); // one row of the covariate bar
    ...
    const align = 1;
    for (let j = 0; j < currentClassBar.height - DET.paddingHeight; j++) {
      // draw the class bar into the dataBuffer
   ...
   **********************************************************************************/
  DET.drawScatterBarPlotColClassBar = function (
    mapItem,
    pixels,
    pos,
    rowClassBarWidth,
    ...
    const matrix = SUM.buildScatterBarPlotMatrix(
      currentClassBar.height - DET.paddingHeight,
      1,
      classBarValues.slice(start - 1, start - 1 + length),
      currentClassBar,
    );
    ...
    const padding =
      mapItem.dataViewWidth - length * mapItem.dataBoxWidth - align;
    if (padding < 0) {
      console.error(
        "Negative padding in drawScatterBarPlotColClassBar",
        padding,
        mapItem.dataViewWidth,
        length,
        mapItem.dataBoxWidth,
      );
    }
  ...
  DET.detailDrawRowClassBars = function (mapItem, pixels) {
    ...
    let offset =
      ((detailTotalWidth * DET.dataViewBorder) / 2) * DRAW.BYTE_PER_RGBA; // start position of very bottom dendro
    bars.forEach((currentClassBar) => {
      ...
      if (
        (mapItem.mode == "RIBBONV" || mapItem.mode == "FULL_MAP") &&
        typeof rowClassBarData[currentClassBar.label].svalues !== "undefined"
      ) {
      ...
      if (currentClassBar.bar_type === "color_plot") {
        pos = DET.drawColorPlotRowClassBar(
          mapItem,
          pixels,
          pos,
          start,
        ...
        pos = DET.drawScatterBarPlotRowClassBar(
          mapItem,
          pixels,
          pos,
          start,
   ...
   * color plot class bars on a given detail heat map canvas.
   **********************************************************************************/
  DET.drawColorPlotRowClassBar = function (
    mapItem,
    pixels,
    pos,
    start,
    ...
    for (let j = start + length - 1; j >= start; j--) {
      // for each row shown in the detail panel
      const val = classBarValues[j - 1];
      const { r, g, b, a } = colorMap.getClassificationColor(val);
      for (let boxRows = 0; boxRows < mapItem.dataBoxHeight; boxRows++) {
        // draw this color to the proper height
        for (let k = 0; k < currentClassBar.height - DET.paddingHeight; k++) {
          // draw this however thick it needs to be
          pixels[pos] = r;
          pixels[pos + 1] = g;
          pixels[pos + 2] = b;
          pixels[pos + 3] = a;
   ...
   * bar and scatter plot class bars on a given detail heat map canvas.
   **********************************************************************************/
  DET.drawScatterBarPlotRowClassBar = function (
    mapItem,
    pixels,
    pos,
    start,
  ...
  function calculateCovariateBarScale(mapItem, axis) {
    ...
    let scale = origScale;
    for (;;) {
      totalBarHeight = Math.max(
        bars.totalHeight(),
        bars.length * (DET.paddingHeight + 1),
      );
      ...
      bars.forEach((bar) => {
        const fontSize =
          Math.max(bar.height - DET.paddingHeight, 1) * fontScale;
  ...
  (function () {
    ...
    DET.detInitGl = function (mapItem) {
      if (!mapItem.glManager) {
        mapItem.glManager = DRAW.GL.createGlManager(
          mapItem.canvas,
          getDetVertexShader,
          getDetFragmentShader,
          () => {

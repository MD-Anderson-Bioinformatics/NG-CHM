(function () {
  ...
  class DetailHeatMapView {
    ...
    isVisible() {
      ...
      return (
        !loc.pane.classList.contains("collapsed") &&
        loc.pane.style.display !== "none"
      );
    }
    ...
    for (let i = 0; i < DVW.detailMaps.length; i++) {
      ...
      if (item.chm === chm) {
        ...
        const makePrimaryButton =
          oldPrimaryLoc.paneHeader.querySelector(".make-primary");
        item.version = "S";
  ...
  DMM.setPrimaryDetailMap = function (mapItem) {
    ...
    if (SUM.rowDendro) {
      ...
      if (mapItem.selectedIsDendrogram && mapItem.mode.startsWith("RIBBONV")) {
        SUM.rowDendro.setRibbonModeBar(
          mapItem.selectedStart,
    ...
    if (SUM.colDendro) {
      ...
      if (mapItem.selectedIsDendrogram && mapItem.mode.startsWith("RIBBONH")) {
        SUM.colDendro.setRibbonModeBar(
          mapItem.selectedStart,
          mapItem.selectedStop,
        );
      }
  ...
  function setSelectionAsDetailView(searchLabels) {
    ...
    if (menuOpenCanvas) {
      ...
      if (mapItem == null) {
        console.error(
          "Cannot find the detail panel on which the menu popup was opened",
        );
        return;
      }
  ...
  function setSubRibbonView(mapItem, startRow, endRow, startCol, endCol) {
    ...
    if (endRow - startRow < 1 && endCol - startCol < 1) {
      ...
      var boxSize = DET.getNearestBoxSize(
        mapItem,
        "row",
        endRow - startRow + 1,
      );
  ...
  (function () {
    ...
    function switchPaneToDetail(loc, restoreInfo) {
      ...
      const isPrimary = restoreInfo
        ? restoreInfo.isPrimary
        : DVW.primaryMap === null;
      ...
      PANE.setPaneClientIcons(
        loc,
        DEV.createClientButtons(mapNumber, paneId, DMM.switchToPrimary),
      );
      const mapItem = DMM.addDetailMap(
        chm,
        paneId,
        mapNumber,
        isPrimary,
      ...
      loc.pane.dataset.intro =
        "This panel shows a detailed (zoomed) view of a section of the heat map.";
    }
	...
	*/
    function constructDetailMapDOMTemplate() {
      ...
      colLabels.setAttribute(
        "style",
        "display: inline-block; position: absolute; right: 0px;",
      );
      colLabels.oncontextmenu = function (event) {
        DEV.labelRightClick(event);
      };
      ...
      rowLabels.setAttribute(
        "style",
        "display: inline-block; position: absolute; bottom: 0px;",
      );
      rowLabels.oncontextmenu = function (event) {
        DEV.labelRightClick(event);
      };

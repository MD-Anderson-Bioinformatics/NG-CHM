(function () {
  ...
  function findPaneLocation(element) {
    ...
    if (!res.container) {
      ...
      console.error({
        m: "findPaneLocation: could not find container",
        element,
        res,
      });
      return res;
    ...
    if (!res.paneHeader) {
      ...
      if (hdrs.length !== 1) {
        console.error({
          m: "findPaneLocation: wrong number of headers",
          res,
          hdrs,
        });
        return res;
    ...
    if (!res.paneTitle) {
      ...
      if (titles.length !== 1) {
        console.error({
          m: "findPaneLocation: wrong number of header titles",
          res,
          titles,
        });
        return res;
  ...
  function initializePanes() {
    ...
    function resizeNGCHM() {
      ...
      if (topContainer && topContainer.parentElement) {
        ...
        const hdrbb = document
          .getElementById("mdaServiceHeader")
        ...
        const newWidth = Math.floor(
          (PANE.ngchmContainerWidth * bb.width) / 100,
        );
        const newHeight =
          Math.floor((PANE.ngchmContainerHeight * bb.height) / 100) -
          hdrbb.height;

        if (debug)
          console.log({
            m: "window.onresize: topContainer",
            oldwidth: oldbb.width,
            newWidth,
            oldheight: oldbb.height,
            newHeight,
          });
  ...
  function openFullScreen(paneId) {
    ...
    origContainer = {
      width: thisPaneParent.style.width,
      height: thisPaneParent.style.height,
    };
  ...
  function addPanelIcons(loc, userIcons) {
    ...
    const paneIcon = loc.paneHeader.querySelector(
      "DIV.icon_group .paneMenuIcon",
    );
  ...
  function clearExistingDialogs(paneId) {
    [...document.querySelectorAll("#" + paneId + " [data-popup-name]")].forEach(
      (icon) => {
  ...
  function resizeHandler(e) {
    ...
    function resizeChildren() {
      if (e.target.classList.contains("ngChmContainer")) {
        ...
        const children = [...e.target.children].filter(
          (ch) =>
            !ch.classList.contains("resizerHelper") &&
            !ch.classList.contains("collapsed"),
        );
        if (verticalContainer !== verticalChange) {
          ...
          children
            ...
            .dispatchEvent(
              new CustomEvent("paneresize", {
                detail: { what: e.detail.what, amount: amt + extra },
              }),
            );
          children.forEach((ch) => {
            ch.dispatchEvent(
              new CustomEvent("paneresize", {
                detail: { what: e.detail.what, amount: amt },
              }),
            );
          });
        }
  ...
  function newPane(style, title, paneid) {
    ...
    if (title) {
      ...
      const img = UTIL.newSvgButton(
        "icon-four-panels!icon-four-panels-glow.paneMenuIcon",
      );
      initializePaneIconMenu(img);
      ig.appendChild(img);

      const expander = UTIL.newSvgButton("icon-expand.expander", {
        align: "left",
      });
      const shrinker = UTIL.newSvgButton("icon-shrink.shrinker", {
        align: "left",
      });
  ...
  function splitPaneCheck(vertical, loc) {
    ...
    if (
      ...
      loc.pane.textContent.indexOf("PathwayMapper") > -1
    ) {
      /**
      ...
      function promisePrompt(vertical, loc) {
        return new Promise(function (resolve, reject) {
          ...
          UHM.setMessageBoxText(
            "This action will delete all the information in PathwayMapper. Would you like to continue?",
          );
      ...
      promisePrompt(vertical, loc)
        .then(function () {
          // promise resolved, split pane
          splitPane(vertical, loc);
        })
        .catch(function (error) {
          // promise rejected, do not split pane
      ...
      // pane division can proceed w/o any loss of PathwayMapper state
      splitPane(vertical, loc);
    }
  } // end splitPaneCheck
  ...
  function splitPane(vertical, loc) {
    ...
    splitter.classList.add(
      "resizerSplitter" + (vertical ? "Horizontal" : "Vertical"),
    );
    ...
    if (
      vertical === verticalContainer &&
      loc.container.id !== "ngChmContainer"
    ) {
    ...
    if (debug)
      console.log({
        m: "child1",
        height: child1.style.height,
        width: child1.style.width,
      });
    if (debug)
      console.log({
        m: "child2",
        height: child2.style.height,
        width: child2.style.width,
      });
  ...
  function newIconMenu(icon) {
    ...
    if (isPaneExpanded === true) {
      ...
      menuItem("Uncollapse", () => {
        ...
        const what = paneLoc.container.classList.contains("vertical")
          ? "height"
          : "width";
        ...
        if (curSize < 200) {
          ...
          if (siblings.length > 0) {
            ...
            siblings.forEach((ch) => {
              ch.dispatchEvent(
                new CustomEvent("paneresize", { detail: resizeDetail }),
              );
            });
          }
          // Then expand target.
          ...
          paneLoc.pane.dispatchEvent(
            new CustomEvent("paneresize", { detail: { what, amount } }),
          );
        }
      ...
      if (getExpandedSibling(paneLoc) == null) {
        ...
        menuItem("Collapse", () => {
          ...
          if (debug)
            console.log({
              width: paneLoc.pane.clientWidth,
              height: paneLoc.pane.clientHeight,
            });
        ...
        menuItem("Close", () => {
          ...
          if (debug)
            console.log({
              ...
              paneLoc,
              parentC,
              siblings: paneLoc.container.children,
            });
          try {
            // remove Gear dialong if it exists
          ...
          function replaceContainerWithOnlyChild() {
            ...
            if (ch.classList.contains("pane")) {
              ...
              if (ch.hasAttribute("data-plugin-name")) {
                paneLoc.container.setAttribute(
                  "data-plugin-name",
                  ch.getAttribute("data-plugin-name"),
                );
              }
              ...
              if (
                pc.id !== "ngChmContainer" &&
                pc.classList.contains("ngChmContainer") &&
                pc.classList.contains("vertical") ===
                  paneLoc.container.classList.contains("vertical")
              ) {
                while (paneLoc.container.firstChild) {
                  pc.insertBefore(
                    paneLoc.container.firstChild,
                    paneLoc.container,
                  );
                }
          ...
          function promisePrompt(paneLoc) {
            return new Promise(function (resolve, reject) {
              ...
              UHM.setMessageBoxHeader("PathwayMapper Pane Reset Warning");
              UHM.setMessageBoxText(
                "This action will delete all information in PathwayMapper. Would you like to continue?",
              );
              UHM.setMessageBoxButton(
                "cancel",
                { type: "text", text: "Cancel" },
          ...
          if (
            paneLoc.container.textContent.indexOf("PathwayMapper") > -1 &&
            c.length < 4
          ) {
            promisePrompt(paneLoc)
              .then(function () {
                // promise resolved, continue pane manipulation
              ...
              .catch(function (error) {
                // promise rejected, do NOT continue pane manipulation
                if (error) {
                  console.error(error);
                }
              })
            ...
            // PathwayMapper pane unaffected, OK to close
            removePaneAndAdjacentDivider();
            if (paneLoc.container.children.length === 1) {
              replaceContainerWithOnlyChild();
            } else {
  ...
  function registerPaneEventHandler(pane, name, callback) {
    if (debug)
      console.log({ m: "registerPaneEventHandler", pane, name, callback });
    // Update existing handler if any
    for (let idx = 0; idx < panesWithEventHandlers.length; idx++) {
      if (
        panesWithEventHandlers[idx] === pane &&
        paneEventNames[idx] === name
      ) {
  ...
  function createInitialPane() {
    ...
    if (topContainer.children.length === 0) {
      ...
      console.error({
        m: "topContainer already initialized",
        bb: topContainer.getBoundingClientRect(),
      });
  ...
  DividerControl.prototype.debouncedDividerMove = function debouncedDividerMove(
    x,
    y,
  ) {
    ...
    if (this.dividerElement.classList.contains("resizerHelperRight")) {
      ...
      if (x !== 0) {
        change = true;
        resizeEvent1 = new CustomEvent("paneresize", {
          detail: { what: "width", amount: x },
        });
        resizeEvent2 = new CustomEvent("paneresize", {
          detail: { what: "width", amount: -x },
        });
      }
    } else {
    ...
    if (debug)
      console.log({
        ...
        x,
        y,
        e1w: element1.style.width,
        e1h: element1.style.height,
        debounceN: this.debounceN,
      });
  ...
  function redistributeContainer(container, spaceTarget) {
    if (debug)
      console.log({ m: "> redistributeContainer", container, spaceTarget });
    ...
    if (container.classList.contains("vertical")) {
      ...
      if (calch2 != Math.round(bb2.height)) {
        if (debug)
          console.log(
            "Target pane is " +
              bb2.height +
              " pixels, but should be " +
              calch2 +
              " pixels",
          );
        const resizeEvent2 = new CustomEvent("paneresize", {
          detail: { what: "height", amount: calch2 - bb2.height },
        });
      ...
      if (calcw2 != Math.round(bb2.width)) {
        ...
        const resizeEvent2 = new CustomEvent("paneresize", {
          detail: { what: "width", amount: calcw2 - bb2.width },
        });
        spaceTarget.dispatchEvent(resizeEvent2);
      }
    }
    ...
    if (summary) {
      const sumPercent = Math.ceil(
        (100 * summary.pane.clientWidth) / UTIL.containerElement.clientWidth,
      );
  ...
  function setPanePropWidths(percent, left, right, divider) {
    const w =
      UTIL.containerElement.clientWidth -
      divider.offsetWidth -
      verticalDividerMargins;
  ...
  function resizePane(pane) {
    ...
    if (debug)
      console.log({ m: "resizePane", title: loc.paneTitle.innerText, loc });
    if (loc.pane.children.length > 1) {
      ...
      loc.pane.children[1].style.height =
        loc.pane.clientHeight - loc.pane.children[0].offsetHeight - 4 + "px";
    }
  ...
  function getStandardConfiguration() {
    ...
    if (c1.length !== 1 || !c1[0].classList.contains("ngChmContainer"))
      return {};
    ...
    if (
      c2.length !== 3 ||
      !c2[0].classList.contains("pane") ||
      !c2[2].classList.contains("pane")
    )
      return {};

(function () {
  ...
  SMM.onMouseUpSelRowCanvas = function (evt) {
    ...
    var sumRow =
      SUM.canvasToMatrixRow(yPos) -
      Math.floor(DVW.getCurrentSumDataPerCol(DVW.primaryMap) / 2);
  ...
  SMM.onMouseUpSelColCanvas = function (evt) {
    ...
    var yPos = SUM.getCanvasY(sumOffsetY);
    var sumCol =
      SUM.canvasToMatrixCol(xPos) -
      Math.floor(DVW.getCurrentSumDataPerRow(DVW.primaryMap) / 2);
  ...
  SMM.onMouseDownCanvas = function (evt) {
    ...
    evt.stopPropagation();
    var boxY =
      (SUM.colClassBarHeight / SUM.canvas.height) * SUM.boxCanvas.height;
    var sumOffsetX = evt.touches
      ? SUM.getTouchEventOffset(evt).offsetX
      : evt.offsetX;
    var sumOffsetY = evt.touches
      ? SUM.getTouchEventOffset(evt).offsetY
      : evt.offsetY;
  ...
  SMM.onMouseUpCanvas = function (evt) {
    if (mouseEventActive) {
      ...
      if (dragSelect) {
        ...
        var clickEndRow = Math.max(
          sumRow * heatMap.getRowSummaryRatio(MAPREP.SUMMARY_LEVEL),
          1,
        );
        var clickEndCol = Math.max(
          sumCol * heatMap.getColSummaryRatio(MAPREP.SUMMARY_LEVEL),
          0,
        );
  ...
  SMM.dragMove = function (evt) {
    ...
    var sumRow =
      SUM.canvasToMatrixRow(yPos) -
      Math.round(DVW.getCurrentSumDataPerCol(DVW.primaryMap) / 2);
    var sumCol =
      SUM.canvasToMatrixCol(xPos) -
      Math.round(DVW.getCurrentSumDataPerRow(DVW.primaryMap) / 2);
    DVW.setCurrentRowFromSum(DVW.primaryMap, sumRow);
    DVW.setCurrentColFromSum(DVW.primaryMap, sumCol);
    DVW.primaryMap.updateSelection();
    SRCH.enableDisableSearchButtons(DVW.primaryMap);
  ...
  SMM.initSummaryEventHandlers = function () {
    ...
    document
      .getElementById("summary_row_select_canvas")
      .addEventListener("mouseup", SMM.onMouseUpSelRowCanvas);
    document
      .getElementById("summary_row_top_items_canvas")
      .addEventListener("mouseup", SMM.onMouseUpSelRowCanvas);
    document
      .getElementById("summary_col_select_canvas")
      .addEventListener("mouseup", SMM.onMouseUpSelColCanvas);
    document
      .getElementById("summary_col_top_items_canvas")
      .addEventListener("mouseup", SMM.onMouseUpSelColCanvas);
  ...
  (function () {
    ...
    function switchPaneToSummary(loc) {
      ...
      loc.pane.dataset.intro =
        "<P>This panel shows an overview of the entire map.  Clicking on this map moves the primary detail panel to that location.</P>";
    }
    ...
    function resizeSummaryPane(loc) {
      if (document.getElementById("summary_chm") == null) {
        return;
      }

(function () {
  ...
  function DetailDendrogram(summaryDendrogram, callbacks) {
    ...
    this.selectWindowBars = function () {
      ...
      if (
        win.startIdx !== this.windowStartIdx ||
        win.numElements !== this.windowNumElements ||
        this.windowBars.length === 0
      ) {
        ...
        this.stop3NIndex = convertMapIndexTo3NSpace(
          win.startIdx + win.numElements,
        );
    ...
    this.buildView = function () {
      ...
      while (true) {
        maxHeight =
          this.summaryDendrogram.dendroViewHeight /
          this.heightRatio /
          this.zoomLevel;
        visBars = 0;
        if (this.barsMaxHeight !== maxHeight || !this.bars) {
          this.bars = this.windowBars.filter((bar) => {
            ...
            return (
              (bar.left >= this.start3NIndex && bar.leftBot < maxHeight) ||
    ...
    this.getScaledView = function () {
      const selectedBarsCheck = JSON.stringify(
        this.summaryDendrogram.selectedBars,
      );
      if (
        !this.scaledView ||
        ...
        this.scaledView = this.bars.map((bar) => {
          ...
          let sidx = 0;
          while (
            sidx < this.summaryDendrogram.selectedBars.length &&
            !selected
          ) {
            const sb = this.summaryDendrogram.selectedBars[sidx];
            selected =
              bar.left >= sb.leftBoundary && bar.right <= sb.rightBoundary;
            sidx++;
          }
    ...
    // event listeners
    this.dendroCanvas.addEventListener(
      "wheel",
      (e) => this.scroll(e),
    ...
    this.scroll = function (e) {
      ...
      if (e.touches) {
        ...
        const touchLoc =
          this.axis === "Column" ? e.touches[0].clientY : e.touches[0].clientX;
    ...
    this.click = function (e) {
      ...
      const w3n =
        (canvasPosn.w * win.numElements + win.startIdx - 1) *
        this.summaryDendrogram.getPointsPerLeaf();
      const h3n =
        (canvasPosn.h * this.summaryDendrogram.dendroViewHeight) / heightScale;
      // console.log ({ w3n, h3n });
      ...
      this.checkDendroHit(h3n, w3n, (barIdx) => {
        ...
        const modified = this.summaryDendrogram.addSelectedBar(
          sumIdx,
          e.shiftKey,
          e.metaKey || e.ctrlKey,
          labelLastClicked,
        ...
        if (modified) {
          const clickType =
            e.ctrlKey || e.metaKey ? "ctrlClick" : "standardClick";
   ...
   **************************/
  DETDDR.DetailColumnDendrogram = function (
    mapItem,
    dendroCanvas,
    summaryDendro,
    callbacks,
    ...
    this.getWindow = function () {
      if (typeof mapItem === "undefined") {
        ...
        return {
          startIdx: 1,
          numElements: mapItem.heatMap.getNumColumns(MAPREP.DETAIL_LEVEL),
        };
      } else {
        return {
          startIdx: mapItem.currentCol,
          numElements: mapItem.dataPerRow,
        };
      }
    };
   ...
   **************************/
  DETDDR.DetailRowDendrogram = function (
    mapItem,
    dendroCanvas,
    summaryDendro,
    callbacks,
    ...
    this.getWindow = function () {
      if (typeof mapItem === "undefined") {
        ...
        return {
          startIdx: 1,
          numElements: mapItem.heatMap.getNumRows(MAPREP.DETAIL_LEVEL),
        };
      } else {
        return {
          startIdx: mapItem.currentRow,
          numElements: mapItem.dataPerCol,
        };
      }
    };

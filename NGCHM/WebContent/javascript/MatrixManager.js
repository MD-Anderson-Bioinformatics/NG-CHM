(function () {
  ...
  class TileWindow {
    constructor(heatMap, tileSpec) {
      ...
      if (
        ...
        console.error(
          "Constructing TileWindow with out-of-range tiles",
          tileSpec,
          heatMapLevel,
        );
      }
      ...
      for (let row = this.startRowTile; row <= this.endRowTile; row++) {
        for (let col = this.startColTile; col <= this.endColTile; col++) {
          const tileCacheName =
            this.layer + "." + this.level + "." + row + "." + col;
          this.tiles[idx] =
            this.heatMap.tileCache.getTileCacheData(tileCacheName);
          if (!this.tiles[idx]) {
    ...
    checkTile(tile) {
      ...
      if (this.isTileInWindow(tile)) {
        const idx =
          (tile.row - this.startRowTile) * this.numColumnTiles +
          tile.col -
          this.startColTile;
    ...
    getTileData(tileRow, tileCol) {
      if (
        ...
        const idx =
          (tileRow - this.startRowTile) * this.numColumnTiles +
          tileCol -
          this.startColTile;
        ...
        for (let col = this.startColTile; col <= this.endColTile; col++) {
          if (!this.tiles[idx]) {
            ...
            this.tiles[idx] =
              this.heatMap.tileCache.getTileCacheData(tileCacheName);
            if (!this.tiles[idx]) {
              this.tiles[idx] = this.heatMap.tileCache.getTile(
                this.layer,
                this.level,
                row,
                col,
  ...
  class TileCache {
    ...
    getTile(layer, level, tileRow, tileColumn) {
      const debug = false;
      const tileCacheName =
        layer + "." + level + "." + tileRow + "." + tileColumn;
      ...
      if (entry) {
        ...
        if (entry.fetchTime > 0.0) {
          ...
          if (
            performance.now() - entry.fetchTime <
            Math.max(3000, 1.25 * this.longestLoadTime)
          ) {
    ...
    setTileCacheEntry(tileCacheName, arrayData) {
      ...
      this.heatMap.updateTileWindows(
        Object.assign({}, entry.props, { data: arrayData }),
      );
    }
  }
  // END class TileCache.
  ...
  class AccessWindow {
    ...
    getValue(row, column) {
      return this.datalevel.getLayerValue(
        this.win.layer,
        this.tileWindow,
        row | 0,
        column | 0,
    ...
    getRowValues(row, firstCol, numCols) {
      ...
      return this.datalevel.getRowValues(
        this.tileWindow,
        row,
        firstCol,
        numCols,
  ...
  //
  {
    ...
    HeatMap.prototype.setSelectionColors = function () {
      const colorMap = this.getColorMapManager().getColorMap(
        "data",
        this._currentDl,
      );
      ...
      root.style.setProperty(
        "--in-selection-background-color",
        dataLayer.selection_color,
      );
    };
    ...
    HeatMap.prototype.getCovariateBarParams = function (axis) {
      return Object.entries(this.getAxisCovariateConfig(axis)).map(
        ([key, config]) => (config.show === "Y" ? barParams(config) : {}),
      );
    };
    ...
    HeatMap.prototype.getRowClassificationOrder = function (showOnly) {
      var rowClassBarsOrder =
        this.mapConfig.row_configuration.classifications_order;
      ...
      if (typeof showOnly === "undefined") {
        ...
        for (var i = 0; i < rowClassBarsOrder.length; i++) {
          ...
          var currConfig =
            this.mapConfig.row_configuration.classifications[newKey];
    ...
    HeatMap.prototype.getColClassificationOrder = function (showOnly) {
      var colClassBarsOrder =
        this.mapConfig.col_configuration.classifications_order;
      // If configuration not found, create order from classifications config
      ...
      if (typeof showOnly === "undefined") {
        ...
        for (var i = 0; i < colClassBarsOrder.length; i++) {
          ...
          var currConfig =
            this.mapConfig.col_configuration.classifications[newKey];
          if (currConfig.show == "Y") {
    ...
    HeatMap.prototype.getScaledVisibleCovariates = function (axis, scale) {
      ...
      const bars = order
        .map(
          (key, idx) =>
            new VisibleCovariateBar(
              key,
              idx,
              axisConfig.classifications[key],
              scale,
            ),
        )
    ...
    HeatMap.prototype.hasHiddenCovariates = function (axis) {
      ...
      return (
        order
          .map((key) => axisConfig.classifications[key].show)
          .filter((show) => show !== "Y").length > 0
      );
    };
    ...
    HeatMap.prototype.setDividerPref = function (sumSize) {
      ...
      if (sumSize === undefined) {
        ...
        sumPercent = Math.ceil(
          (100 * summary.clientWidth) / container.clientWidth,
        );
      } else {
        sumPercent = sumSize;
      }
      this.mapConfig.data_configuration.map_information.summary_width =
        sumPercent;
      this.mapConfig.data_configuration.map_information.detail_width =
        100 - sumPercent;
    };

    HeatMap.prototype.setClassificationPrefs = function (
      classname,
      axis,
      showVal,
      heightVal,
      ...
      if (MAPREP.isRow(axis)) {
        ...
        this.mapConfig.col_configuration.classifications[classname].show =
          showVal ? "Y" : "N";
        this.mapConfig.col_configuration.classifications[classname].height =
          parseInt(heightVal);
      }
    };
    ...
    HeatMap.prototype.setClassBarScatterPrefs = function (
      ...
      if (MAPREP.isRow(axis)) {
        ...
        if (typeof lowBound !== "undefined") {
          this.mapConfig.row_configuration.classifications[
            classname
          ].low_bound = lowBound;
          this.mapConfig.row_configuration.classifications[
            classname
          ].high_bound = highBound;
          this.mapConfig.row_configuration.classifications[classname].fg_color =
            fgColorVal;
          this.mapConfig.row_configuration.classifications[classname].bg_color =
            bgColorVal;
        }
      } else {
        this.mapConfig.col_configuration.classifications[classname].bar_type =
          barType;
        if (typeof lowBound !== "undefined") {
          this.mapConfig.col_configuration.classifications[
            classname
          ].low_bound = lowBound;
          this.mapConfig.col_configuration.classifications[
            classname
          ].high_bound = highBound;
          this.mapConfig.col_configuration.classifications[classname].fg_color =
            fgColorVal;
          this.mapConfig.col_configuration.classifications[classname].bg_color =
            bgColorVal;
        }
      }
    };
    ...
    HeatMap.prototype.setLayerGridPrefs = function (
      ...
      this.mapConfig.data_configuration.map_information.data_layer[
        key
      ].grid_show = showVal ? "Y" : "N";
      this.mapConfig.data_configuration.map_information.data_layer[
        key
      ].grid_color = gridColorVal;
      this.mapConfig.data_configuration.map_information.data_layer[
        key
      ].cuts_color = gapColorVal;
      this.mapConfig.data_configuration.map_information.data_layer[
        key
      ].selection_color = selectionColorVal;
     ...
     */
    HeatMap.prototype.saveDataSentToPluginToMapConfig = function (
      nonce,
      postedConfig,
      postedData,
    ) {
      try {
        var pane = document.querySelectorAll('[data-nonce="' + nonce + '"]')[0]
      ...
      if (postedConfig)
        this.mapConfig.panel_configuration[paneId].config = postedConfig;
      if (postedData)
        this.mapConfig.panel_configuration[paneId].data = postedData;
      this.mapConfig.panel_configuration[paneId].type = "plugin";
      this.mapConfig.panel_configuration[paneId].pluginName =
        pane.dataset.pluginName;
    };
     ...
     */
    HeatMap.prototype.saveLinkoutPaneToMapConfig = function (
      paneid,
      url,
      paneTitle,
    ) {
	...
	*/
    HeatMap.prototype.saveDataFromPluginToMapConfig = function (
      nonce,
      dataFromPlugin,
    ) {
      try {
        var paneId = document.querySelectorAll(
          '[data-nonce="' + nonce + '"]',
        )[0].parentElement.parentElement.id;
      } catch (err) {
        throw "Cannot determine pane for given nonce";
        return false;
      }
      ...
      this.mapConfig.panel_configuration[paneId].dataFromPlugin =
        dataFromPlugin;
    };
    ...
    HeatMap.prototype.getAllAlternateLevels = function (level) {
      ...
      function getAlternates(level) {
        const altlevs = alternateLevels.hasOwnProperty(level)
    ...
    HeatMap.prototype.tileIdReferenced = function (tileId) {
      ...
      this.tileWindowRefs.forEach((value, key) => {
        const [tlayer, tlevel, firstRow, lastRow, firstCol, lastCol] =
          key.split(".");
        if (
          tlayer == layer &&
          tlevel == level &&
    ...
    HeatMap.prototype.hasGaps = function () {
      return (
        this.getMapInformation().map_cut_rows +
          this.getMapInformation().map_cut_cols !=
        0
      );
    };
    ...
    HeatMap.prototype.sendCallBack = function sendCallBack(event, tile) {
      //Initialize event
      if (
        event === MMGR.Event_INITIALIZED ||
        event === MMGR.Event_JSON ||
        ...
        if (data) {
          this.sendAllListeners(MMGR.Event_NEWDATA, {
  ...
  function prefetchInitialTiles(heatMap) {
    const datalayers =
      heatMap.mapConfig.data_configuration.map_information.data_layer;
  ...
  class HeatMapLevel {
    constructor(tileCache, level, jsonData, lowerLevel) {
      ...
      this.rowToLower =
        lowerLevel === null ? null : this.totalRows / lowerLevel.totalRows;
      this.colToLower =
        lowerLevel === null
          ? null
          : this.totalColumns / lowerLevel.totalColumns;
    ...
    numRowsInTile(tileRow) {
      return tileRow < this.numTileRows
        ? this.rowsPerTile
        : this.rowsInLastTile;
    }
    ...
    numColsInTile(tileCol) {
      return tileCol < this.numTileColumns
        ? this.colsPerTile
        : this.colsInLastTile;
    }
    ...
    getLayerValue(layer, tileWindow, row, column) {
      ...
      if (arrayData) {
        ...
        return arrayData[
          ((row - 1) % this.rowsPerTile) * thisTileColsPerRow +
            ((column - 1) % this.colsPerTile)
        ];
    ...
    getRowValues(tileWindow, row, firstCol, numCols) {
      ...
      myIterable[Symbol.iterator] = function* () {
        for (let nextCol = 0; nextCol < numCols; ) {
          ...
          if (arrayData) {
            const idx =
              rowIndexInTile * thisTileColsPerRow +
              ((firstCol + nextCol - 1) % colsPerTile);
            ...
            const firstLowerCol =
              (((firstCol + nextCol - 1) * invColToLower) | 0) + 1;
            const lastLowerCol =
              (((firstCol + nextCol + colsLeftInTile - 2) * invColToLower) |
                0) +
              1;
            ...
            for (let { i: lowI, col: lowCol, value } of lowerVals) {
              const endOfUpperPixel = Math.min(
                ((lowI + 1) * colToLower) | 0,
                colsLeftInTile,
              );
  ...
  function loadNgChmFromZip(heatMap, addMapConfig, addMapData) {
    ...
    reader
      ...
      .then(function (entries) {
        ...
        if (
          mapConfigName in heatMap.zipFiles &&
          mapDataName in heatMap.zipFiles
        ) {
          setTimeout(() => {
            zipFetchJson(
              heatMap,
              heatMap.zipFiles[mapConfigName],
              addMapConfig,
            );
          });
          setTimeout(() => {
  ...
  HeatMap.prototype.loadHeatMapTile = function loadHeatMapTile(job) {
    ...
    if (cacheData) {
      if (debug)
        console.log("Request for tile currently in cache: ", { tileCacheName });
      return;
    }
    if (debug)
    ...
    if (this.currentTileRequests.includes(tileCacheName)) {
      if (debug)
        console.log(
          "Request for tile we are currently reading: " + tileCacheName,
        );
      return;
    }
    let idx = this.pendingTileRequests
      .map((p) => p.tileCacheName)
      .indexOf(tileCacheName);
    if (idx >= 0) {
      // Remove it so we can put it on the end.
      if (debug)
        console.log("Request for tile in pending queue: " + tileCacheName);
    ...
    if (debug) {
      ...
      const pendingBaseNames = this.pendingTileRequests.map(
        (p) => p.tileCacheName,
      );
    ...
    if (this.currentTileRequests.length > 10) {
      if (debug)
        console.log(
          "Too many tiles being read: " + this.currentTileRequests.length,
        );
      return;
    }
  ...
  // cannot be obtained.
  HeatMap.prototype.tileRequestComplete = function tileRequestComplete(
    tileCacheName,
    tileData,
  ) {
    const debug = false;
  ...
  // discarded.
  //
  HeatMap.prototype.submitFirstPendingTileRequest =
    function submitFirstPendingTileRequest() {
      const debug = false;
      ...
      let job =
        this.pendingTileRequests.length == 0
          ? null
          : this.pendingTileRequests.pop();
      while (job && !this.tileIdReferenced(job.tileCacheName)) {
        if (debug)
          console.log(
            "Abandoning request for unreferenced tile: " + job.tileCacheName,
          );
        job =
          this.pendingTileRequests.length == 0
            ? null
      ...
      if (
        this.source() == MMGR.WEB_SOURCE ||
        this.source() == MMGR.LOCAL_SOURCE
      ) {

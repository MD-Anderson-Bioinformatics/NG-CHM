(function () {
  ...
  PDF.openPdfPrefs = function (e) {
    ...
    if (SUM.isVisible() && !DVW.anyVisible()) {
      ...
      UHM.systemMessage(
        "NG-CHM PDF",
        "Cannot generate PDF: inconsistent visibility check",
      );
      return;
    }
    updateShowBounds();
    ...
    if (pdfPrefsHdr.querySelector(".closeX")) {
      pdfPrefsHdr.querySelector(".closeX").remove();
    }
    ...
    prefspanel.style.left =
      (window.innerWidth - prefspanel.offsetWidth) / 2 + "px";
  };
    ...
    generatePDF(heatMap)
      ...
      .catch((error) => {
        ...
        if (error instanceof PdfError) {
          ...
          message =
            "Unexpected error detected during PDF generation: " + message + ".";
          if (showResolutionHint) {
            message +=
              "<P>If this a large map, try using a smaller resolution (DPI).</P>";
          }
          message += `<P>Please consider making or contributing to a bug report on our
  ...
  function generatePDF(heatMap) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const mapsToShow = isChecked("pdfInputSummaryMap")
        ? "S"
        : isChecked("pdfInputDetailMap")
          ? "D"
          : "B";
      ...
      function addSummaryPage(pdfDoc) {
        return new Promise((resolve, reject) => {
          drawSummaryHeatMapPage(
            pdfDoc,
            includeDetailMaps && pdfDoc.showDetailBounds,
          );
          resolve();
        });
      }
...

  {
    ...
    pdfResolution.onchange = function (e) {
      const pdfCustomResLabel = document.querySelector(
    ...
    pdfCustomResolution.dataset.tooltip =
      "Enter DPI value between " + minDPI + " and " + maxDPI;
    ...
    function checkResolution() {
      ...
      if (isNaN(val)) {
        ...
        createPdfButton.dataset.disabledReason =
          "Disabled because custom DPI is not a number.";
      ...
      if (val < minDPI) {
        createPdfButton.disabled = true;
        createPdfButton.dataset.disabledReason =
          "Disabled because custom DPI is less than " + minDPI + ".";
        pdfCustomResolution.style.backgroundColor = "#ff00004d";
        return;
      }
      ...
      if (val > maxDPI) {
        createPdfButton.disabled = true;
        createPdfButton.dataset.disabledReason =
          "Disabled because custom DPI is greater than " + maxDPI + ".";
        pdfCustomResolution.style.backgroundColor = "#ff00004d";
        return;
      }
  ...
  function getPdfDocument(heatMap) {
    ...
    if (isNaN(this.resolution)) {
      console.warn(
        "Unexpected non-numeric pdf resolution. Defaulting to 600 dpi.",
      );
  ...
  function calcLongestLabelUnits(doc, allLabels, axis, fontSize) {
    ...
    allLabels.forEach((label) => {
      if (label.dataset.axis == axis) {
        longest = Math.max(
          doc.getStringUnitWidth(label.innerHTML) * fontSize,
          longest,
        );
      }
    });
  ...
  function setTopItemsSizing(doc, maxFontSize) {
    ...
    for (let i = 0; i < rowTopItems.length; i++) {
      longestRowTopItems = Math.max(
        doc.getStringUnitWidth(rowTopItems[i]),
        longestRowTopItems,
      );
    }
    longestRowTopItems *= maxFontSize * 1.05;
    for (let i = 0; i < colTopItems.length; i++) {
      longestColTopItems = Math.max(
        doc.getStringUnitWidth(colTopItems[i]),
        longestColTopItems,
      );
    }
  ...
  function setSummaryDendroDimensions(
    ...
    const rowDendroPctg =
      rowDendroW / (W(SUM.boxCanvas) + W(SUM.rCCanvas) + rowDendroW);
    const colDendroH = H(document.getElementById("column_dendro_canvas"));
    const colDendroPctg =
      colDendroH / (H(SUM.boxCanvas) + H(SUM.cCCanvas) + colDendroH);
  ...
  function setSummaryClassDimensions(
    ...
    const rowDendroW = W(document.getElementById("row_dendro_canvas"));
    const rowClassBarPctg =
      W(SUM.rCCanvas) / (W(SUM.boxCanvas) + W(SUM.rCCanvas) + rowDendroW);
    const colDendroH = H(document.getElementById("column_dendro_canvas"));
    const colClassBarPctg =
      H(SUM.cCCanvas) / (H(SUM.boxCanvas) + H(SUM.cCCanvas) + colDendroH);
  ...
  function setSummaryHeatmapDimensions(
    ...
    const rowDendroW = W(document.getElementById("row_dendro_canvas"));
    const sumMapWPctg =
      W(SUM.boxCanvas) / (W(SUM.boxCanvas) + W(SUM.rCCanvas) + rowDendroW);
    const colDendroH = H(document.getElementById("column_dendro_canvas"));
    const sumMapHPctg =
      H(SUM.boxCanvas) / (H(SUM.boxCanvas) + H(SUM.cCCanvas) + colDendroH);
  ...
  function createHeader(titleText, options = {}) {
    ...
    if (
      logo &&
      PDF.ngchmLogo &&
      (PDF.isWidget === false || typeof isNgChmAppViewer !== "undefined")
    ) {
      ...
      doc.text(
        10,
        10,
        "NG-CHM Heat Map: " + heatMap.getMapInformation().name,
        null,
      );
      doc.setFillColor(255, 0, 0);
      doc.setDrawColor(255, 0, 0);
  ...
  function isChecked(el) {
    if (document.getElementById(el)) return document.getElementById(el).checked;
  }
  ...
  function initLegends() {
      ...
      return accessWindow.onready().then((win) => {
        ...
        for (var i = 1; i < numCol + 1; i++) {
          for (var j = 1; j < numRow + 1; j++) {
            ...
            if (isNaN(val) || val >= MAPREP.maxValues) {
              // is it Missing value?
              nan++;
            } else if (val <= MAPREP.minValues) {
              // is it a cut location?
              continue;
            }
            if (val <= breaks[0]) {
              bins[0]++;
        ...
        var threshMaxLen = getThreshMaxLength(
          thresholds,
          barsInfo.classBarLegendTextSize,
        );
        var missingCount = 0;

        var barHeight = barsInfo.classBarLegendTextSize + 3;
        for (var j = 0; j < breaks.length; j++) {
          // draw all the bars within the break points
          ...
          if (barsInfo.options.condenseClassBars) {
            ...
            doc.text(
              leftOff +
                threshMaxLen -
                doc.getStringUnitWidth(breaks[j].toString()) *
                  barsInfo.classBarLegendTextSize -
                4,
              bartop + barsInfo.classBarLegendTextSize + barHeight / 2,
              breaks[j].toString(),
              null,
            );
            doc.text(
              leftOff + barW + threshMaxLen + 10,
              bartop + barsInfo.classBarLegendTextSize,
              "n = " + value + " (" + ((value / total) * 100).toFixed(2) + "%)",
              null,
            );
          } else {
            // histogram
            var barW = (value / binMax) * barsInfo.classBarFigureW * 0.65; //scale bars to fit page
        ...
        if (missingCount > 0) {
          ...
          doc.setFillColor(rgb.r, rgb.g, rgb.b);
          doc.setDrawColor(0, 0, 0);
          const barW = barsInfo.options.condenseClassBars
            ? 10
            : (missingCount / binMax) * barsInfo.classBarFigureW;
          doc.rect(leftOff + threshMaxLen, bartop, barW, barHeight, "FD");
          doc.setFontSize(barsInfo.classBarLegendTextSize);
          doc.text(
            leftOff +
              threshMaxLen -
              doc.getStringUnitWidth("Missing Value") *
                barsInfo.classBarLegendTextSize -
              4,
            bartop + barsInfo.classBarLegendTextSize,
            "Missing Value",
          ...
          doc.text(
            ...
            "n = " +
              missingCount +
              " (" +
              ((missingCount / total) * 100).toFixed(2) +
              "%)",
            null,
          );
        }
    ...
    function drawRowClassLegends(pdfDoc, barsInfo) {
      ...
      if (barsInfo.rowBarsToDraw.length > 0) {
        barsInfo.leftOff = 20; // ...reset leftOff...
        const topSkip =
          barsInfo.classBarFigureH +
          classBarHeaderSize +
          barsInfo.classBarTitleSize +
          20; // return class bar height to original value in case it got changed in this row
        ...
        for (let i = 0; i < barsInfo.rowBarsToDraw.length; i++) {
          ...
          const colorCount = isDiscrete
            ? barConfig.color_map.colors.length
            : 10;
          if (i === 0) {
    ...
    function drawLegendSubSectionHeader(pdfDoc, barsInfo, categories, key) {
      ...
      if (splitTitle.length > 1) {
        barsInfo.classBarHeaderHeight =
          classBarHeaderSize * splitTitle.length + 4 * splitTitle.length + 10;
      }
      if (
        barsInfo.topOff + barsInfo.classBarHeaderHeight + categories * 13 >
        pdfDoc.pageHeight
      ) {
     ...
     **********************************************************************************/
    function getBarGraphForDiscreteClassBar(
      pdfDoc,
      key,
      type,
      barsInfo,
      ...
      const doc = pdfDoc.doc;
      var barScale = isChecked("pdfInputPortrait") ? 0.5 : 0.65;
      var foundMissing = 0;
      ...
      var bartop =
        barsInfo.topOff +
        5 +
        (splitTitle.length - 1) * barsInfo.classBarLegendTextSize * 2;
      var colorMap = pdfDoc.heatMap.getColorMapManager().getColorMap(type, key);
      var thresholds = colorMap.getThresholds();
      var maxLabelLength =
        doc.getStringUnitWidth("XXXXXXXXXXXXXXXX") *
        barsInfo.classBarLegendTextSize;
      if (isChecked("pdfInputPortrait") && thresholds.length > 56) {
        ...
        doc.text(
          barsInfo.leftOff + 15,
          bartop + barsInfo.classBarLegendTextSize,
          "This discrete covariate bar contains too",
          null,
        );
        doc.text(
          barsInfo.leftOff + 15,
          bartop + barsInfo.classBarLegendTextSize + 12,
          "many categories to print.",
          null,
        );
        ...
        doc.text(
          barsInfo.leftOff + 15,
          bartop + barsInfo.classBarLegendTextSize + 12,
          "many categories to print. You may try",
          null,
        );
        doc.text(
          barsInfo.leftOff + 15,
          bartop + barsInfo.classBarLegendTextSize + 24,
        ...
        if (
          barsInfo.topOff +
            barsInfo.classBarHeaderHeight +
            thresholds.length * 13 >
            pdfDoc.pageHeight &&
          !isLastClassBarToBeDrawn(pdfDoc.heatMap, key, type)
        ) {
          doc.addPage(); // ... make a new page and reset topOff
          pdfDoc.createHeader(null, {
            contText: barsInfo.sectionHeader + " (continued)",
          });
          barsInfo.topOff = pdfDoc.paddingTop + 15;
          barsInfo.leftOff = 20; // ...reset leftOff...
        }
        ...
        for (var val in counts) {
          ...
          maxLabelLength = Math.max(
            maxLabelLength,
            doc.getStringUnitWidth(val) * barsInfo.classBarLegendTextSize,
          );
        }
        ...
        for (var j = 0; j < thresholds.length; j++) {
          // make a gradient stop (and also a bucket for continuous)
          ...
          if (barsInfo.options.condenseClassBars) {
            ...
            doc.setFontSize(barsInfo.classBarLegendTextSize);
            doc.text(
              barsInfo.leftOff + barW + 5,
              bartop + barsInfo.classBarLegendTextSize,
              thresholds[j].toString(),
              null,
            );
            doc.text(
              barsInfo.leftOff + barW + threshMaxLen + 10,
              bartop + barsInfo.classBarLegendTextSize,
              "n = " +
                count +
                " (" +
                ((count / classBarData.values.length) * 100).toFixed(2) +
                "%)",
              null,
            );
          } else {
            ...
            doc.rect(
              barsInfo.leftOff + maxLabelLength,
              bartop,
              barW,
              barHeight,
              "FD",
            ...
            doc.text(
              barsInfo.leftOff +
                maxLabelLength -
                doc.getStringUnitWidth(thresholds[j].toString()) *
                  barsInfo.classBarLegendTextSize -
                4,
              bartop + barsInfo.classBarLegendTextSize,
              thresholds[j].toString(),
            ...
            doc.text(
              barsInfo.leftOff + maxLabelLength + barW + 5,
              bartop + barsInfo.classBarLegendTextSize,
              "n = " +
                count +
                " (" +
        ...
        if (missingCount > 0) {
          ...
          drawMissingColor(
            ...
            maxLabelLength,
            threshMaxLen,
            classBarData.values.length,
          );
        }

        if (thresholds.length * barHeight > barsInfo.classBarFigureH) {
          // in case a discrete classbar has over 15 categories, make the topOff increment bigger
          barsInfo.topSkip =
            (thresholds.length + 1) * barHeight + classBarHeaderSize;
        }
        setClassBarFigureH(
          barsInfo,
          thresholds.length,
     ...
     **********************************************************************************/
    function getBarGraphForContinuousClassBar(
      pdfDoc,
      key,
      type,
      barsInfo,
      ...
      if (
        barsInfo.topOff + barsInfo.classBarHeaderHeight + 130 >
        pdfDoc.pageHeight
      ) {
        doc.addPage(); // ... make a new page and reset topOff
        pdfDoc.createHeader(null, {
      ...
      for (let i = 0; i < classBarData.values.length; i++) {
        if (classBarData.values[i] === "!CUT!") {
          ...
          if (isNaN(num)) {
            console.warn(
              "Encountered bad continuous covariate value in " +
                key +
                ": " +
                classBarData.values[i],
            );
          } else {
            let k;
            if (num <= thresholds[0]) {
      ...
      for (var val in counts) {
        ...
        maxLabelLength = Math.max(
          maxLabelLength,
          doc.getStringUnitWidth("" + val.length) *
            barsInfo.classBarLegendTextSize,
        );
      }
      ...
      for (var j = 0; j < thresholds.length; j++) {
        const rgb =
          classBar.bar_type === "color_plot"
            ? colorMap.getClassificationColor(thresholds[j])
            : black;
        doc.setFillColor(rgb.r, rgb.g, rgb.b);
        doc.setDrawColor(0, 0, 0);
        let value = counts[j];
        ...
        if (barsInfo.options.condenseClassBars) {
          ...
          doc.text(
            ...
            "n = " +
              value +
              " (" +
              ((value / classBarData.values.length) * 100).toFixed(2) +
              "%)",
            null,
          );
        } else {
          ...
          doc.text(
            barsInfo.leftOff +
              maxLabelLength -
              doc.getStringUnitWidth(valLabel) *
                barsInfo.classBarLegendTextSize -
              4,
            bartop + barsInfo.classBarLegendTextSize,
            valLabel,
    ...
    function drawMissingColor(
      ...
      const doc = pdfDoc.doc;
      const barScale = isChecked("pdfInputPortrait") ? 0.5 : 0.65;
      if (barsInfo.options && barsInfo.options.condenseClassBars) {
        ...
        doc.text(
          ...
          "n = " +
            missingCount +
            " (" +
            ((missingCount / totalValues) * 100).toFixed(2) +
            "%)",
          null,
        );
      } else {
        var barW =
          (missingCount / maxCount) * barsInfo.classBarFigureW * barScale;
        doc.rect(
          barsInfo.leftOff + maxLabelLength,
          bartop,
          barW,
     ...
     **********************************************************************************/
    function adjustForNextClassBar(
      pdfDoc,
      barsInfo,
      key,
      type,
      ...
      if (barsInfo.leftOff + barsInfo.classBarFigureW > pdfDoc.pageWidth) {
        // if the next class bar figure will go beyond the width of the page...
        barsInfo.leftOff = 20; // ...reset leftOff...
        barsInfo.topSkip =
          barsInfo.classBarFigureH +
          barsInfo.classBarHeaderHeight +
          10 * barsInfo.covTitleRows;
        barsInfo.covTitleRows = 1;
        ...
        var nextClassBarFigureH = getNextLineClassBarFigureH(
          pdfDoc,
          barsInfo,
          key,
          type,
        ...
        if (
          ...
          // if the next class bar goes off the page vertically...
          pdfDoc.addPageIfNeeded(); // ... make a new page and reset topOff
          pdfDoc.createHeader(null, {
            contText: barsInfo.sectionHeader + " (continued)",
          });
    ...
    function getNextLineClassBarFigureH(pdfDoc, barsInfo, key, type) {
      var minLabelLength =
        pdfDoc.doc.getStringUnitWidth("Missing Value") *
        barsInfo.classBarLegendTextSize;
      var classBarsToDraw =
        type == "col" ? barsInfo.colBarsToDraw : barsInfo.rowBarsToDraw;
      var classBars =
        type == "col"
          ? pdfDoc.heatMap.getColClassificationConfig()
          : pdfDoc.heatMap.getRowClassificationConfig();
      ...
      var nextIdx = index + 1;
      while (
        numFigures * (barsInfo.classBarFigureW + minLabelLength + 60) <
        pdfDoc.pageWidth
      ) {
        ...
        if (thisBar.color_map.type == "continuous") {
          threshCount = 10;
        }
  ...
  function drawSummaryHeatMapPage(pdfDoc, showDetailViewBounds) {
    ...
    const {
      ...
      rowTopItemsLength,
      colTopItemsLength,
    } = setTopItemsSizing(pdfDoc.doc, topItemsFontSize);
    const sumPaddingWidth = 5;
    const sumPaddingHeight = 5;
    const sumImgW =
      pdfDoc.doc.getPageWidth() - pdfDoc.paddingLeft - sumPaddingWidth; //width of available space for heatmap, class bars, and dendro
    const sumImgH =
      pdfDoc.doc.getPageHeight() - pdfDoc.paddingTop - sumPaddingHeight; //height of available space for heatmap, class bars, and dendro

    //Get Dimensions for Summary Row & Column Dendrograms
    const { rowDendroWidth, colDendroHeight } = setSummaryDendroDimensions(
      sumImgW,
      sumImgH,
      rowTopItemsLength,
      colTopItemsLength,
    ...
    const { rowClassWidth, colClassHeight } = setSummaryClassDimensions(
      sumImgW,
      sumImgH,
      rowTopItemsLength,
      colTopItemsLength,
    ...
    const { sumMapW, sumMapH } = setSummaryHeatmapDimensions(
      sumImgW,
      sumImgH,
      rowTopItemsLength,
      colTopItemsLength,
    ...
    const mapWidthScale = Math.max(
      1,
      Math.ceil(
        ((pdfDoc.resolution / stdPDFRes) * sumImgW) /
          heatMap.getNumColumns(MAPREP.SUMMARY_LEVEL),
      ),
    );
    const mapHeightScale = Math.max(
      1,
      Math.ceil(
        ((pdfDoc.resolution / stdPDFRes) * sumImgH) /
          heatMap.getNumRows(MAPREP.SUMMARY_LEVEL),
      ),
    );
    ...
    const rowClassLeft =
      rowDendroLeft +
      (rowDendroConfig.show !== "NONE" ? rowDendroWidth + 1 : 0);
    ...
    const colClassTop =
      colDendroTop +
      (colDendroConfig.show !== "NONE" ? colDendroHeight + 1 : 0);
    ...
    if (rowDendroConfig.show !== "NONE") {
      if (debug)
        console.log("rowDendro@", {
          left: rowDendroLeft,
          top: imgTop,
          width: rowDendroWidth,
          height: sumMapH,
        });
      SUM.rowDendro.drawPDF(pdfDoc.doc, {
        left: rowDendroLeft,
        top: imgTop,
        width: rowDendroWidth,
        height: sumMapH,
    ...
    if (SUM.rCCanvas.width > 0) {
      ...
      const rowCovBarSize = heatMap
        .getScaledVisibleCovariates("row", 1.0)
        .totalHeight();
      const rowCovWidthScale = Math.max(
        1,
        Math.ceil(
          ((pdfDoc.resolution / stdPDFRes) * rowClassWidth) / rowCovBarSize,
        ),
      );
      const renderBuffer = SUM.buildRowCovariateRenderBuffer(
        rowCovWidthScale,
        mapHeightScale,
      );
    ...
    if (colDendroConfig.show !== "NONE") {
      if (debug)
        console.log("colDendro@", {
          left: imgLeft,
          top: colDendroTop,
          width: sumMapW,
          height: colDendroHeight,
        });
      SUM.colDendro.drawPDF(pdfDoc.doc, {
        left: imgLeft,
        top: colDendroTop,
        width: sumMapW,
        height: colDendroHeight,
    ...
    if (SUM.cCCanvas.height > 0) {
      ...
      const colCovBarSize = heatMap
        .getScaledVisibleCovariates("column", 1.0)
        .totalHeight();
      const colCovWidthScale = Math.max(
        1,
        Math.ceil(
          ((pdfDoc.resolution / stdPDFRes) * colClassHeight) / colCovBarSize,
        ),
      );
      const renderBuffer = SUM.buildColCovariateRenderBuffer(
        mapWidthScale,
        colCovWidthScale,
      );
      ...
      if (debug)
        console.log("colCovar@", {
          left: imgLeft,
          top: colClassTop,
          width: sumMapW,
          height: colClassHeight,
        });
      pdfDoc.doc.addImage(
        sumColClassData,
        "PNG",
    ...
    {
      const renderBuffer = SUM.renderHeatMapToRenderBuffer(
        mapWidthScale,
        mapHeightScale,
      );
    ...
    if (SUM.rowTopItemPosns.length > 0 || SUM.colTopItemPosns.length > 0) {
      ...
      drawSummaryTopItemLabels(pdfDoc, "row", {
        left: imgLeft + sumMapW + topItemsWidth + 2,
        top: imgTop,
        width: undefined,
        height: sumMapH,
      });
      drawSummaryTopItemLabels(pdfDoc, "col", {
        left: imgLeft,
        top: imgTop + sumMapH + topItemsHeight + 2,
        width: sumMapW,
        height: undefined,
      });
    }
  ...
  function drawSummaryTopItemLabels(pdfDoc, axis, vp) {
    ...
    const topItems = [...document.getElementsByClassName("topItems")].filter(
      (ti) => ti.axis === axis,
    );
    if (debug) {
    ...
    if (axis === "row") {
      topItems.forEach((item) => {
        ...
        doc.text(
          vp.left,
          vp.top + vp.height * relPosn + labelFontSize * 0.75,
          item.innerHTML,
        );
        if (debug) {
          console.log("Drew row label " + item.innerHTML, {
            x: vp.left,
            y: vp.top + vp.height * relPosn + labelFontSize,
            relPosn: relPosn,
            rect,
          });
        }
      ...
      topItems.forEach((item) => {
        ...
        doc.text(
          vp.left + vp.width * relPosn + labelFontSize / 2,
          vp.top,
          item.innerHTML,
          null,
          270,
        ...
        if (debug) {
          console.log("Drew col label " + item.innerHTML, {
            x: vp.left + vp.width * relPosn + labelFontSize / 2,
            y: vp.top,
            relPosn: relPosn,
            rect,
          });
  ...
  function addDetailPage(pdfDoc, mapItem) {
    ...
    return DVW.getDetailWindow(mapItem)
      .onready()
      .then((win) => {
        const origFontSize = doc.getFontSize();
        pdfDoc.addPageIfNeeded();
        ...
        const mapLabelDivs = Object.entries(mapItem.labelElements).map(
          ([id, label]) => label.div,
        );
        const rowLabels = mapLabelDivs.filter(
          (label) => label.dataset.axis == "Row",
        );
        const colLabels = mapLabelDivs.filter(
          (label) => label.dataset.axis == "Column",
        );
        const columnCovarLabels = mapLabelDivs.filter(
          (label) => label.dataset.axis == "ColumnCovar",
        );
        const rowCovarLabels = mapLabelDivs.filter(
          (label) => label.dataset.axis == "RowCovar",
        );

        const labelFontSize = getFontSizeForLabels(
          rowLabels,
          mapItem.rowLabelFont,
          colLabels,
          mapItem.colLabelFont,
        );

        const allLabels = [
          ...mapItem.chm.getElementsByClassName("DynamicLabel"),
        ];
        const longestRowLabelUnits = calcLongestLabelUnits(
          doc,
          allLabels,
          "Row",
          labelFontSize,
        );
        const longestColLabelUnits = calcLongestLabelUnits(
          doc,
          allLabels,
          "Column",
          labelFontSize,
        ...
        const longestRowCovarUnits = calcLongestLabelUnits(
          doc,
          allLabels,
          "RowCovar",
          mapItem.rowClassLabelFont,
        );
        const longestColCovarUnits = calcLongestLabelUnits(
          doc,
          allLabels,
          "ColumnCovar",
        ...
        const detImgH =
          pdfDoc.pageHeight -
          pdfDoc.paddingTop -
          Math.max(longestColLabelUnits, longestRowCovarUnits) -
          paddingBottom;
        const detImgW =
          pdfDoc.pageWidth -
          pdfDoc.paddingLeft -
          Math.max(longestRowLabelUnits, longestColCovarUnits) -
          paddingRight;
        ...
        const rowDendroPctg =
          rowDendroConfig.show == "ALL"
            ? rcw / (mapItem.canvas.clientWidth + rcw)
            : 0;
        const colDendroPctg =
          colDendroConfig.show == "ALL"
            ? cch / (mapItem.canvas.clientHeight + cch)
            : 0;
        ...
        const detRowClassWidth =
          detMapW * (rowCovariates.totalHeight() / mapItem.canvas.width);
        const colCovariates = mapItem.getScaledVisibleCovariates("column");
        const detColClassHeight =
          detMapH * (colCovariates.totalHeight() / mapItem.canvas.height);
        ...
        {
          const { drawWin, params } =
            mapItem.detailHeatMapParams[mapItem.heatMap._currentDl];
        ...
        const mapWidthScale = Math.max(
          1,
          Math.ceil(
            ((pdfDoc.resolution / stdPDFRes) * detMapW) /
              heatMap.getNumColumns(level),
          ),
        );
        const mapHeightScale = Math.max(
          1,
          Math.ceil(
            ((pdfDoc.resolution / stdPDFRes) * detMapH) /
              heatMap.getNumRows(level),
          ),
        );
        ...
        const boxDoc = doc.beginFormObject(
          0,
          0,
          detMapW + 1e-5,
          detMapH + 1e-5,
        ...
        const boxMatrix2 = new doc.Matrix(
          1.0,
          0.0,
          0.0,
          1.0,
          imgLeft,
          pdfDoc.pageHeight - (imgTop + detMapH),
        );
        ...
        Object.assign(mapItemVars, {
          labelFontSize,
          longestRowLabelUnits,
          longestColLabelUnits,
          detClient2PdfWRatio,
          detClient2PdfHRatio,
          detRowDendroWidth,
          detColDendroHeight,
      ...
      rowLabels.forEach((label) => {
        if (SRCHSTATE.labelIndexInSearch("Row", label.dataset.index)) {
          const { x, y } = calcRowLabelPosn(
            ...
            mapItemVars,
            label,
            mapItemVars.labelFontSize,
          );
          doc.rect(
            x,
            y - mapItemVars.labelFontSize,
            mapItemVars.longestRowLabelUnits + 2,
            mapItemVars.labelFontSize,
            "F",
          );
        }
      ...
      colLabels.forEach((label) => {
        if (SRCHSTATE.labelIndexInSearch("Column", label.dataset.index)) {
          ...
          doc.rect(
            x - 2,
            y,
            mapItemVars.labelFontSize + 2.5,
            mapItemVars.longestColLabelUnits + 2,
            "F",
          );
        }
      });
    ...
    function calcRowLabelPosn(mapItem, mapItemVars, label, labelFontSize) {
      ...
      const left =
        pdfDoc.paddingLeft + mapItemVars.detRowDendroWidth + labelFontSize / 2;
      // The -3 in the following line undoes a corresponding +3 when positioning the label in DET/drawAxisLabels.
      const x =
        left +
        (label.offsetLeft - mapItem.canvas.offsetLeft - 3) /
          mapItemVars.detClient2PdfWRatio;

      const top =
        pdfDoc.paddingTop + mapItemVars.detColDendroHeight + labelFontSize;
      let y =
        top +
        (label.offsetTop - mapItem.canvas.offsetTop) /
          mapItemVars.detClient2PdfHRatio;
    ...
    function calcColLabelPosn(mapItem, mapItemVars, label, labelFontSize) {
      ...
      const y =
        (label.offsetTop - mapItem.canvas.offsetTop) /
          mapItemVars.detClient2PdfHRatio +
        pdfDoc.paddingTop +
        mapItemVars.detColDendroHeight;
      if (label.id.indexOf("legendDet") > -1) {
